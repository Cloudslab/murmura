...
def test_privacy_budget_dataclass():
    ...
    assert (
        abs(budget.utilization_percentage - 110.0) < 1e-10
...
def test_record_aggregation_round():
    ...
    record = accountant.record_aggregation_round(
        num_clients=5, noise_multiplier=1.0, round_number=1
    )
...
def test_privacy_summary():
    ...
    privacy_record = PrivacySpent(
        epsilon=1.0, delta=1e-5, round_number=1, client_id="client_0"
    )
...
def test_export_privacy_history():
    ...
    record1 = PrivacySpent(
        epsilon=1.0, delta=1e-5, round_number=1, client_id="client_0"
    )
    record2 = PrivacySpent(
        epsilon=1.5, delta=1.5e-5, round_number=2, client_id="client_1"
    )
...
def test_suggest_optimal_noise():
    ...
    noise_multiplier = accountant.suggest_optimal_noise(
        sample_rate=0.01, epochs=5, dataset_size=60000
    )
...
def test_compute_privacy_spent():
    ...
    epsilon, delta = accountant.compute_privacy_spent(
        noise_multiplier=1.0, sample_rate=0.01, steps=1000
    )
