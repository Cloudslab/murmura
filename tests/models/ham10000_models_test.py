...
class TestHAM10000Model:
    ...
    def test_init_standard_architecture(self):
        ...
        has_batchnorm = any(
            isinstance(layer, (nn.BatchNorm2d, nn.BatchNorm1d))
    ...
    def test_init_dp_compatible_architecture(self):
        ...
        has_groupnorm = any(
            isinstance(layer, nn.GroupNorm) for layer in model.modules()
        )
        has_layernorm = any(
            isinstance(layer, nn.LayerNorm) for layer in model.modules()
        )
        has_batchnorm = any(
            isinstance(layer, (nn.BatchNorm2d, nn.BatchNorm1d))
            for layer in model.modules()
...
class TestSimpleHAM10000Model:
    ...
    def test_no_normalization_layers(self):
        ...
        has_norm = any(
            isinstance(
                layer, (nn.BatchNorm2d, nn.BatchNorm1d, nn.GroupNorm, nn.LayerNorm)
            )
            for layer in model.modules()
        )
        assert not has_norm
...
class TestModelCompatibility:
    ...
    def test_models_inherit_from_pytorch_model(self):
        ...
        models = [HAM10000Model(), HAM10000ModelComplex(), SimpleHAM10000Model()]
