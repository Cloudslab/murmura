...
def test_invalid_num_actors():
    ...
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            num_actors=0, feature_columns=["image"], label_column="label"
        )

    with pytest.raises(ValidationError):
        OrchestrationConfig(
            num_actors=-5, feature_columns=["image"], label_column="label"
        )
...
def test_invalid_min_partition_size():
    ...
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            min_partition_size=0, feature_columns=["image"], label_column="label"
        )

    with pytest.raises(ValidationError):
        OrchestrationConfig(
            min_partition_size=-10, feature_columns=["image"], label_column="label"
        )
...
def test_config_inheritance_and_override():
    ...
    base = OrchestrationConfig(
        num_actors=2, feature_columns=["image"], label_column="label"
    )
...
def test_empty_feature_columns():
    ...
    with pytest.raises(
        ValidationError, match="feature_columns must be a non-empty list"
    ):
...
def test_empty_label_column():
    ...
    with pytest.raises(
        ValidationError, match="label_column must be a non-empty string"
    ):
...
def test_label_in_features():
    ...
    with pytest.raises(
        ValidationError, match="label_column .* cannot be included in feature_columns"
    ):
...
def test_multinode_validation_actors_per_node():
    ...
    with pytest.raises(
        ValidationError, match="num_actors .* cannot be less than actors_per_node"
    ):
        OrchestrationConfig(
            num_actors=5,
...
def test_training_parameters_validation():
    ...
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            batch_size=0, feature_columns=["image"], label_column="label"
        )

    # Invalid learning_rate
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            learning_rate=0.0, feature_columns=["image"], label_column="label"
        )
...
def test_sampling_parameters():
    ...
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            client_sampling_rate=0.005, feature_columns=["image"], label_column="label"
        )

    with pytest.raises(ValidationError):
        OrchestrationConfig(
            data_sampling_rate=1.5, feature_columns=["image"], label_column="label"
        )
...
def test_monitoring_parameters():
    ...
    with pytest.raises(ValidationError):
        OrchestrationConfig(
            health_check_interval=0, feature_columns=["image"], label_column="label"
        )
...
def test_get_resource_requirements_none_values():
    ...
    config = OrchestrationConfig(
        resources=ResourceConfig(
            cpus_per_actor=None, gpus_per_actor=None, memory_per_actor=None
        ),
        feature_columns=["image"],
        label_column="label",
    )
...
def test_ray_address_backward_compatibility():
    ...
    # Test None case
    config_none = OrchestrationConfig(feature_columns=["image"], label_column="label")
...
def test_test_split_parameter():
    ...
    config = OrchestrationConfig(
        test_split="validation", feature_columns=["image"], label_column="label"
    )
