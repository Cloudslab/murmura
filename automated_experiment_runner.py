...
class ExperimentMetricsCollector(TrainingObserver):
    ...
    def on_event(self, event: TrainingEvent) -> None:
        ...
        if isinstance(event, InitialStateEvent):
            ...
            transfer_data = {
                ...
                "param_summary": event.param_summary.copy()
                if event.param_summary
                else {},
            }
    ...
    def get_experiment_results(self) -> Dict[str, Any]:
        ...
        actual_rounds = (
            max(self.accuracy_history.keys()) if self.accuracy_history else 0
        )
        ...
        # Communication and efficiency metrics
        avg_active_nodes = (
            np.mean(list(self.active_nodes_per_round.values()))
            if self.active_nodes_per_round
            else 0
        )
        communication_efficiency = (
            self.total_parameter_transfers / max(1, actual_rounds)
            if actual_rounds > 0
            else 0
        )
        ...
        convergence_trend = (
            ...
            and self.convergence_history[-1] < self.convergence_history[0]
            else "stable"
        )
        ...
        training_stability = (
            1.0 / (1.0 + np.std(accuracy_improvements))
        ...
        return {
            ...
            "rounds_with_improvement": len([i for i in accuracy_improvements if i > 0])
            if accuracy_improvements
            else 0,
            "max_single_round_improvement": max(accuracy_improvements)
            if accuracy_improvements
            else 0.0,
            ...
            "parameter_history_summary": {
                node: {
                    "count": len(params),
                    "final": params[-1] if params else 0.0,
                    "trend": "stable"
                    if len(params) < 2
                    else ("increasing" if params[-1] > params[0] else "decreasing"),
                }
...
class EnhancedExperimentRunner:
    ...
    def generate_experiment_configs(
        ...
        self.logger.info(
            f"Generated {len(configs)} configurations for {experiment_set} experiment set"
        )
        return configs

    def run_single_experiment_with_observer(
        self, config: ExperimentConfig, experiment_id: str
    ) -> ExperimentResult:
        """
        ...
        self.logger.info(
            f"Starting experiment {experiment_id}: {config.paradigm}-{config.dataset}-{config.topology}"
        )
        ...
        try:
            ...
            if config.paradigm == "decentralized":
                ...
                from murmura.data_processing.partitioner_factory import (
                    PartitionerFactory,
                )
                ...
                from murmura.network_management.topology import (
                    TopologyConfig,
                    TopologyType,
                )
                from murmura.node.resource_config import (
                    RayClusterConfig,
                    ResourceConfig,
                )
                ...
                if config.dataset == "mnist":
                    from murmura.examples.decentralized_mnist_example import (
                        MNISTModel,
                        create_mnist_preprocessor,
                    )
                    ...
                    from murmura.examples.decentralized_skin_lesion_example import (
                        WideResNet,
                        create_skin_lesion_preprocessor,
                        add_integer_labels_to_dataset,
                    )
                    from murmura.model.pytorch_model import TorchModelWrapper
                    import torch.nn as nn
                    import torch
                    ...
                    dx_categories, num_classes, dx_to_label = (
                        add_integer_labels_to_dataset(train_dataset, self.logger)
                    )
                    ...
                    raise ValueError(
                        f"Unsupported dataset for decentralized learning: {config.dataset}"
                    )
                ...
                orchestration_config = OrchestrationConfig(
                    ...
                    aggregation=AggregationConfig(
                        strategy_type=AggregationStrategyType(
                            config.aggregation_strategy
                        ),
                        params={"mixing_parameter": 0.5}
                        if config.aggregation_strategy == "gossip_avg"
                        else None,
                    ),
                ...
                raise NotImplementedError(
                    f"Observer-based execution not yet implemented for {config.paradigm}"
                )
            ...
            if "error" not in collected_metrics:
                result = ExperimentResult(
                    ...
                    round_metrics=[
                        collected_metrics
                    ],  # Store all enhanced metrics in round_metrics
                )
                self.logger.info(
                    f"Experiment {experiment_id} completed successfully in {duration:.1f} minutes"
                )
            else:
                result = ExperimentResult(
                    config=config,
    ...
    def run_single_experiment_subprocess(
        ...
        start_time = datetime.now()
        self.logger.info(
            f"Starting experiment {experiment_id} via subprocess: {config.paradigm}-{config.dataset}-{config.topology}"
        )
    ...
    def run_single_experiment(
        ...
        try:
            ...
            if config.paradigm == "decentralized" and config.dataset in [
                "mnist",
                "ham10000",
            ]:
                ...
                self.logger.info(
                    f"Using subprocess approach for {config.paradigm} paradigm"
                )
    ...
    @staticmethod
    def get_example_script_path(
        config: ExperimentConfig, experiment_id: str = None
    ) -> str:
        """Get the appropriate example script path"""
        base_path = "murmura/examples"
        dataset_script_map = {"mnist": "mnist", "ham10000": "skin_lesion"}
    ...
    def build_command(self, config: ExperimentConfig, experiment_id: str) -> List[str]:
        ...
        cmd = [
            ...
            str(config.scale),
            "--partition_strategy",
            "dirichlet" if config.paradigm != "centralized" else "iid",
            "--alpha",
            str(config.alpha),
            "--min_partition_size",
            "50",
        ...
        cmd.extend(
            ["--ray_namespace", f"murmura_exp_{experiment_id}", "--auto_detect_cluster"]
        )
        ...
        if config.privacy != "none":
            cmd.extend(
                [
                    ...
                    "0.8" if config.paradigm != "decentralized" else "1.0",
                ]
            )
    ...
    def parse_experiment_output(
        self, output: str, config: ExperimentConfig
    ) -> Dict[str, Any]:
    ...
    def result_to_dict(self, result: ExperimentResult) -> Dict[str, Any]:
        ...
        if result.round_metrics and len(result.round_metrics) > 0:
            ...
            first_metrics = (
                result.round_metrics[0]
                if isinstance(result.round_metrics, list)
                else result.round_metrics
            )
            ...
            if isinstance(first_metrics, dict):
                enhanced_metrics = [
                    ...
                    "topology_type",
                    "num_nodes",
                    "avg_active_nodes_per_round",
                    "node_participation_rate",
                    "total_parameter_transfers",
                    "parameter_transfers_per_round",
                    "total_local_training_events",
                    "total_aggregation_events",
                    "total_model_updates",
                    "final_parameter_convergence",
                    "avg_parameter_convergence",
                    "parameter_diversity",
                    "convergence_trend",
                    "training_stability",
                    "rounds_with_improvement",
                    "max_single_round_improvement",
                ]
                ...
                enhanced_defaults = {
                    "loss_reduction": result.initial_loss - result.final_loss
                    if result.initial_loss and result.final_loss
                    else 0.0,
                    "actual_rounds": result.convergence_rounds,
                    "time_per_round": result.training_time_minutes
                    / max(1, result.convergence_rounds)
                    if result.training_time_minutes and result.convergence_rounds
                    else 0.0,
                    ...
                    if (result.initial_loss - result.final_loss) > 0
                    else "stable",
                    ...
                    "rounds_with_improvement": result.convergence_rounds
                    if result.accuracy_improvement > 0
                    else 0,
                    "max_single_round_improvement": result.accuracy_improvement
                    / max(1, result.convergence_rounds)
                    if result.accuracy_improvement > 0
                    else 0.0,
                }
            ...
            if output_file.endswith(".xlsx"):
                if file_exists:
                    try:
                        ...
                        self.logger.warning(
                            f"Error reading existing Excel file: {e}. Creating new file."
                        )
    ...
    def run_experiments(
        self, experiment_set: str, output_file: str, resume: bool = False
    ):
        ...
        if resume and os.path.exists(output_file):
            try:
                ...
                configs = [
                    c
                    for c in configs
                    if self.get_experiment_id(c) not in completed_experiment_ids
                ]

                self.logger.info(
                    f"Resuming: {len(completed_experiment_ids)} experiments already completed"
                )
        ...
        for config in configs:
            if (
                ...
                and config.scale <= 10
                and config.privacy == "none"
                and config.heterogeneity == "iid"
            ):
        ...
        for i, config in enumerate(all_configs):
            ...
            self.logger.info(
                f"Progress: {i + 1}/{len(all_configs)} - Running {experiment_id}"
            )
            ...
            if i < 3 and not result.success:
                self.logger.error("Early failure detected. Stopping experiment suite.")
                self.logger.error(
                    f"Fix the issue with {experiment_id} before continuing."
                )
                break
        ...
        if failed_experiments > 0:
            ...
            for result in results:
                if not result.success:
                    ...
                    self.logger.info(
                        f"  {exp_id}: {result.error_type} - {result.error_message}"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Enhanced Murmura Experiment Runner with Observer Pattern"
    )
    ...
    parser.add_argument(
        "--ray_address", type=str, default=None, help="Ray cluster head node address"
    )
    ...
    parser.add_argument(
        "--resume", action="store_true", help="Resume from existing results file"
    )
    ...
    for exp_set in experiment_sets:
        ...
        results = runner.run_experiments(
            experiment_set=exp_set, output_file=output_file, resume=args.resume
        )
