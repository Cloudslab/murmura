...
class AttackConfig(BaseModel):
    ...
    attack_type: Optional[
        Literal["label_flipping", "gradient_manipulation", "both"]
    ] = Field(default=None, description="Type of poisoning attack to perform")
    ...
    attack_intensity_end: float = Field(
        default=1.0, ge=0.0, le=1.0, description="Final attack intensity"
    )

    intensity_progression: Literal["linear", "exponential", "step"] = Field(
        default="linear", description="How attack intensity increases over rounds"
    )
    ...
    label_flip_source: Optional[int] = Field(
        default=None, description="Source label to flip from (if None, flip any label)"
    )

    # Gradient manipulation parameters
    gradient_noise_scale: float = Field(
        default=1.0, ge=0.0, description="Scale factor for gradient noise injection"
    )
    ...
    log_attack_details: bool = Field(
        default=True, description="Enable detailed logging of attack actions"
    )
