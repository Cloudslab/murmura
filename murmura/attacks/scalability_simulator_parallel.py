...
def run_scalability_experiments_parallel(
                ...
                for dp_setting in dp_settings:
                    ...
                    try:
                        ...
                        experiment_configs.append(
                            (config, experiment_key, experiment_id)
                        )
                        experiment_id += 1
    ...
    print(
        f"   Total experiments: {total_experiments} ({len(experiment_configs)} to run, {len(completed_experiments)} already completed)"
    )
    ...
    if num_workers == 1:
        ...
        for i, args in enumerate(experiment_configs):
            ...
            print(
                f"📊 Experiment {exp_id + 1} ({progress_pct:.1f}% complete): "
        ...
        with ProcessPoolExecutor(max_workers=num_workers) as executor:
            ...
            for i, future in enumerate(as_completed(future_to_config)):
                ...
                progress_pct = (
                    (len(completed_experiments) + i + 1) / total_experiments * 100
                )
                ...
                try:
                    ...
                    print(
                        f"📊 [{progress_pct:5.1f}%] Experiment {exp_id + 1}: "
                        f"{config.num_nodes} nodes, {config.topology}, {config.attack_strategy}, "
                    ...
                    print(
                        f"❌ [{progress_pct:5.1f}%] Experiment {exp_id + 1} failed: {e}"
                    )
                    all_results.append(
                        {
