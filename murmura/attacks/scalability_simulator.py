...
class NetworkConfig:
    ...
    attack_strategy: (
        str  # 'sensitive_groups', 'topology_correlated', 'imbalanced_sensitive'
    )
    ...
    def __post_init__(self):
        ...
        if self.attack_strategy not in [
            "sensitive_groups",
            "topology_correlated",
    ...
    def _validate_topology_compatibility(self):
        ...
        if self.topology in ["star"]:
            ...
            if self.attack_strategy == "topology_correlated":
                self.fl_type = (
                    "federated"  # Use federated for better correlation patterns
                )
            else:
        ...
        if self.topology not in valid_topologies:
            raise ValueError(
                f"Topology '{self.topology}' is not compatible with FL type '{self.fl_type}'. "
...
class SyntheticDataGenerator:
        ...
        if self.config.attack_strategy == "sensitive_groups":
            ...
            for node_id in range(self.config.num_nodes):
                if node_id < group_size:
                    ...
                    base_norm = self.rng.uniform(
                        base_norm_range[1] * 0.95, base_norm_range[1]
                    )
                    param_std = self.rng.uniform(
                        param_std_range[0], param_std_range[0] * 1.1
                    )
                    group_id = 0
                else:
                    # Group 2: Lower norms (matching real data patterns)
                    base_norm = self.rng.uniform(
                        base_norm_range[0], base_norm_range[0] * 1.05
                    )
                    param_std = self.rng.uniform(
                        param_std_range[1] * 0.9, param_std_range[1]
                    )
                    group_id = 1
                ...
                characteristics[node_id] = {
                    ...
                    "param_mean": self.rng.uniform(
                        -0.02, 0.08
                    ),  # Real parameter mean range
                }
            ...
            for node_id in range(self.config.num_nodes):
                ...
                if self.config.topology == "star" and node_id == 0:
                    ...
                    base_norm = self.rng.uniform(
                        base_norm_range[0], base_norm_range[0] * 1.02
                    )
                    param_std = self.rng.uniform(
                        param_std_range[0], param_std_range[0] * 1.05
                    )
                    ...
                    base_norm = (
                        base_norm_range[0]
                        + (base_norm_range[1] - base_norm_range[0]) * position_factor
                    )
                    param_std = (
                        param_std_range[0]
                        + (param_std_range[1] - param_std_range[0]) * position_factor
                    )
            ...
            if self.config.num_nodes <= 10:
                ...
                data_ratios = [0.4] + [0.6 / (self.config.num_nodes - 1)] * (
            ...
            for node_id in range(self.config.num_nodes):
                ...
                base_norm = base_norm_range[0] + (
                    base_norm_range[1] - base_norm_range[0]
                ) * min(1.0, norm_scale)
                param_std = (
                    param_std_range[0]
                    + (param_std_range[1] - param_std_range[0]) * data_ratio
                )
    ...
    def generate_parameter_updates(
        self, node_characteristics: Dict[int, Dict[str, float]]
    ) -> pd.DataFrame:
        ...
        for round_num in range(self.config.num_rounds):
            for node_id in range(self.config.num_nodes):
                ...
                param_std_actual = param_std_base * self.rng.uniform(
                    0.8, 1.2
                )  # Add some variation
    ...
    def generate_communications(self) -> pd.DataFrame:
        ...
        if self.config.topology == "star":
            base_comms_per_round = max(
                6, int(24 * (self.config.num_nodes / 5))
            )  # Scale from real 5-node data
        elif self.config.topology == "ring":
        ...
        for round_num in range(self.config.num_rounds):
            ...
            target_communications = int(
                base_comms_per_round * self.rng.uniform(0.8, 1.2)
            )
            ...
            while round_communications < target_communications:
                ...
                for source, target in edges:
                    ...
                    if self.rng.random() < (comm_prob / len(edges)):
                        ...
                        timestamp = base_time + self.rng.uniform(
                            0, 50
    ...
    def generate_full_simulation_data(self) -> Dict[str, pd.DataFrame]:
        ...
        simulation_data = {
            ...
            "node_characteristics": pd.DataFrame.from_dict(
                node_characteristics, orient="index"
            )
            .reset_index()
...
class ScalabilityAnalyzer:
    ...
    def analyze_complexity_scaling(
        self, network_sizes: List[int], topology: str, attack_strategy: str
    ) -> Dict[str, Any]:
    ...
    def predict_attack_effectiveness(
        self, network_size: int, baseline_effectiveness: Dict[int, float], topology: str
    ) -> float:
        ...
        if topology == "complete":
            ...
            predicted = max(
                ...
                + scaling_factor * (math.log(network_size) - math.log(sizes[-1])),
            )
            ...
            predicted = max(
                0.0,
                effectiveness_values[-1]
                + scaling_factor * (math.sqrt(network_size) - math.sqrt(sizes[-1])),
            )
...
class LargeScaleAttackSimulator:
    ...
    def simulate_attack_execution(self) -> Dict[str, Any]:
        ...
        evaluation = self._evaluate_attack_comprehensive(
            attack_results,
            {
                ...
                "dp_setting": {
                    ...
                    "name": f"dp_eps_{self.config.dp_epsilon}"
    ...
    def _evaluate_attack_comprehensive(
        self, attack_results: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        evaluation.update(
            self._analyze_attack_strategy_scalability(attack_results, config)
        )

        # Add topology-specific metrics
        evaluation["topology_specific_metrics"] = (
            self._analyze_topology_effects_scalability(attack_results, config)
        )

        # Add DP effectiveness metrics
        evaluation["dp_effectiveness_metrics"] = (
            self._analyze_dp_effectiveness_scalability(attack_results, config)
        )

        # Add scalability metrics
        evaluation["scalability_metrics"] = (
            self._analyze_scalability_effects_scalability(attack_results, config)
        )
    ...
    def _analyze_attack_strategy_scalability(
        self, attack_results: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _analyze_topology_effects_scalability(
        self, attack_results: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        metrics = {
            ...
            "communication_complexity": self._calculate_communication_complexity_scalability(
                topology, node_count
            ),
        }
    ...
    def _calculate_communication_complexity_scalability(
    ...
    def _analyze_dp_effectiveness_scalability(
        self, attack_results: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            metrics.update(
                {
                    ...
                    "privacy_budget_utilization": self._estimate_privacy_budget_usage_scalability(
                        config
                    ),
                }
            )
    ...
    def _estimate_privacy_budget_usage_scalability(
    ...
    def _analyze_scalability_effects_scalability(
        self, attack_results: List[Dict[str, Any]], config: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    def _calculate_network_complexity(self) -> Dict[str, float]:
        ...
        return {
            ...
            / (self.config.num_nodes * (self.config.num_nodes - 1) / 2),
        }
    ...
    def _calculate_theoretical_bounds(self) -> Dict[str, float]:
        ...
        if self.config.attack_strategy == "sensitive_groups":
            ...
            theoretical_upper_bound = 1.0 - 1.0 / math.sqrt(
                n
            )  # Groups become more distinguishable
            ...
            if self.config.topology == "star":
                ...
                theoretical_upper_bound = 1.0 - 1.0 / math.log(
                    n
...
def run_scalability_experiments(
    ...
    total_experiments = (
        len(network_sizes) * len(topologies) * len(attack_strategies) * len(dp_settings)
    )
    skipped_count = 0
        ...
        for topology in topologies:
            for attack_strategy in attack_strategies:
                for dp_setting in dp_settings:
                    ...
                    progress_pct = (
                        (len(completed_experiments) + skipped_count)
                        / total_experiments
                        * 100
                    )
                    print(
                        f"📊 Experiment {current_experiment_num} ({progress_pct:.1f}% complete): {size} nodes, {topology}, {attack_strategy}, DP={dp_setting.get('name', 'off')}"
                    )
                    ...
                    try:
                        ...
                        max_signal = eval_result.get("attack_indicators", {}).get(
                            "max_signal", 0.0
                        )
                        print(
                            f"   ✅ Attack {'SUCCESS' if attack_success else 'FAILED'} (signal: {max_signal:.3f})"
                        )
...
def analyze_scalability_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        data_points.append(
            {
                ...
                "max_signal": evaluation.get("attack_indicators", {}).get(
                    "max_signal", 0.0
                ),
            }
        )
    ...
    for topology in df["topology"].unique():
        ...
        size_analysis = (
            topo_data.groupby("num_nodes")
            .agg({"attack_success": ["mean", "count"], "max_signal": ["mean", "std"]})
            .round(3)
        )
    ...
    topo_vuln = (
        df.groupby("topology")
        .agg({"attack_success": "mean", "max_signal": "mean"})
    ...
    if "dp_enabled" in df.columns:
        dp_comparison = df.groupby("dp_enabled").agg(
            {"attack_success": "mean", "max_signal": "mean"}
        )

        analysis["dp_effectiveness"] = {
            "without_dp": dp_comparison.loc[False].to_dict()
            if False in dp_comparison.index
            else {},
            "with_dp": dp_comparison.loc[True].to_dict()
            if True in dp_comparison.index
            else {},
