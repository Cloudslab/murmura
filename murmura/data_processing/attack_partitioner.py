    ...
    def _create_ring_correlation(self, targets, unique_classes):
        ...
        for node_id in range(min(self.num_partitions, len(unique_classes))):
            ...
            if n_secondary > 0:
                ...
                secondary_chunks = np.array_split(
                    cls_indices[n_primary:], len(other_nodes)
                )
    ...
    def partition(
        ...
        for cls, indices in rare_indices.items():
            ...
            if n_for_rare_nodes > 0:
                rare_chunks = np.array_split(
                    indices[:n_for_rare_nodes], len(self.rare_class_nodes)
                )
            ...
            if n_for_other_nodes > 0:
                non_rare_nodes = [
                    ...
                    if i not in self.rare_class_nodes
                ]
                if non_rare_nodes:  # Only distribute if there are non-rare nodes
                    remaining_chunks = np.array_split(
                        indices[n_for_rare_nodes:], len(non_rare_nodes)
                    )
        ...
        non_rare_nodes = [
            i for i in range(self.num_partitions) if i not in self.rare_class_nodes
        ]
        ...
        for cls, indices in common_indices.items():
            ...
            if non_rare_nodes:
                ...
                if n_for_non_rare > 0:
                    non_rare_chunks = np.array_split(
                        indices[:n_for_non_rare], len(non_rare_nodes)
                    )
                ...
                if n_for_rare > 0 and self.rare_class_nodes:
                    rare_chunks = np.array_split(
                        indices[n_for_non_rare:], len(self.rare_class_nodes)
                    )
                    for node, chunk in zip(self.rare_class_nodes, rare_chunks):
                        chunk_as_ints = [int(x) for x in chunk]
                        self.partitions[node].extend(chunk_as_ints)
