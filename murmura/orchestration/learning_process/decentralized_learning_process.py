...
class DecentralizedLearningProcess(LearningProcess):
    ...
    def _initialize_trust_monitoring(self) -> None:
        ...
        if (
            not hasattr(self.config, "trust_monitoring")
            or not self.config.trust_monitoring
        ):
        ...
        for node_idx in range(len(self.cluster_manager.actors)):
            ...
            trust_monitor.add_event_callback(
                lambda event: self.training_monitor.emit_event(event)
            )
            ...
            self.logger.info(
                f"Initialized trust monitor for honest node {node_idx} with {len(neighbors)} neighbors"
            )

        self.logger.info(
            f"Trust monitoring initialized for {len(self.trust_monitors)} honest nodes"
        )

    def _process_trust_monitoring(
        self,
        round_num: int,
        ...
        for node_idx, trust_monitor in self.trust_monitors.items():
            ...
            for neighbor_idx in neighbors:
                if neighbor_idx in node_params:
                    ...
                    if neighbor_idx < len(train_metrics):
                        neighbor_losses[f"node_{neighbor_idx}"] = train_metrics[
                            neighbor_idx
                        ].get("loss", 0.0)
            ...
            if neighbor_updates:
                self.logger.info(
                    f"Node {node_idx}: Processing {len(neighbor_updates)} neighbor updates: {list(neighbor_updates.keys())}"
                )
    ...
    def execute(self) -> Dict[str, Any]:
        ...
        for round_num in range(1, rounds + 1):
            ...
            for i, actor in enumerate(self.cluster_manager.actors):
                ...
                if i in self.cluster_manager.malicious_client_indices:
                    ...
                    self.logger.debug(
                        f"Round {round_num}: Collected parameters from MALICIOUS node {i}"
                    )
                else:
                    params = ray.get(actor.get_model_parameters.remote(), timeout=1800)
                    self.logger.debug(
                        f"Round {round_num}: Collected parameters from honest node {i}"
                    )
                ...
                if params:
                    param_count = sum(
                        p.numel() if hasattr(p, "numel") else np.prod(np.array(p).shape)
                        for p in params.values()
                    )
                    self.logger.debug(
                        f"  Node {i}: {len(params)} layers, {param_count} total parameters"
                    )
            ...
            self.logger.info(
                f"Round {round_num}: Processing trust monitoring for {len(self.trust_monitors)} monitors"
            )

            # Log malicious clients for debugging
            if self.cluster_manager.malicious_client_indices:
                self.logger.info(
                    f"Round {round_num}: Known malicious clients: {list(self.cluster_manager.malicious_client_indices)}"
                )

            trust_scores = self._process_trust_monitoring(
                round_num, node_params, train_metrics, node_malicious_detections
            )
            self.logger.info(
                f"Round {round_num}: Trust monitoring returned {len(trust_scores)} results"
            )
            if trust_scores:
                trust_monitoring_results.append(
                    {"round": round_num, "trust_scores": trust_scores}
                )
            ...
            if (
                self.trust_config
                and self.trust_config.enable_trust_weighted_aggregation
                and trust_scores
            ):
                trust_scores_for_aggregation = trust_scores
                self.logger.info(
                    f"Round {round_num}: Enabling trust-weighted aggregation"
                )
            else:
                self.logger.info(
                    f"Round {round_num}: Using standard aggregation (trust weighting disabled)"
                )
        ...
        node_detections_for_logging = {
            node_idx: list(detections)
            for node_idx, detections in node_malicious_detections.items()
        }

        # Update global detection status
        if all_detected_suspicious:
            self.logger.info(
                f"Trust monitoring detected {len(all_detected_suspicious)} suspicious neighbors across all nodes: {list(all_detected_suspicious)}"
            )
            self.logger.info(f"Node-wise detections: {node_detections_for_logging}")
        else:
            self.logger.info(
                "Trust monitoring: No malicious behavior detected by any node"
            )
        ...
        results = {
            ...
            "trust_monitoring": {
                ...
                "global_suspicious_detected": list(all_detected_suspicious)
                if self.trust_monitors
                else [],
                "node_detections": node_detections_for_logging
                if self.trust_monitors
                else {},
            },
        }
