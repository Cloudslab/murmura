...
class PrivacyAccountant:
    ...
    def compute_privacy_spent(
        ...
        self.logger.debug(
            f"Approximate privacy: ε={epsilon:.4f} for {steps} steps "
            f"(σ={sigma}, q={q})"
        )
    ...
    def get_remaining_budget(self, client_id: Optional[str] = None) -> Tuple[float, float]:
        """
        ...
        if client_id is None:
            ...
            if client_id not in self.client_budgets:
                return self.dp_config.target_epsilon, self.dp_config.target_delta or 1e-5
            budget = self.client_budgets[client_id]
    ...
    def get_privacy_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "total_rounds": len(set(r.round_number for r in self.privacy_history if r.round_number)),
    ...
    def suggest_optimal_noise(
        ...
        noise_multiplier = max(0.1, min(10.0, noise_multiplier))  # Clamp to reasonable range
