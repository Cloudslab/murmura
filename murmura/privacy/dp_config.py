...
class DifferentialPrivacyConfig(BaseModel):
    ...
    per_client_clipping: bool = Field(
        default=False, description="Enable per-client clipping for user-level privacy"
    )

    max_clients_per_user: int = Field(
        default=1, ge=1, description="Maximum clients per user (for user-level privacy)"
    )

    # Monitoring and debugging
    enable_privacy_monitoring: bool = Field(
        default=True, description="Enable real-time privacy budget tracking"
    )
    ...
    def validate_dp_config(self) -> "DifferentialPrivacyConfig":
        ...
        if (
            self.clipping_strategy != ClippingStrategy.FIXED
            and self.clipping_norm is None
        ):
            self.clipping_norm = 1.0  # Default for adaptive strategies

        # Client sampling validation
        if (
            self.client_sampling_rate is not None
            and self.noise_application == NoiseApplication.CLIENT_SIDE
        ):
        ...
        if (
            self.accountant in [DPAccountant.RDP, DPAccountant.ZCDP]
            and self.total_rounds is None
        ):
            import warnings
    ...
    def estimate_utility_impact(self) -> Dict[str, str]:
        ...
        if self.epsilon <= 0.1:
            ...
            impact["utility_impact"] = (
                "High - significant accuracy degradation expected"
            )
        elif self.epsilon <= 1.0:
            impact["privacy_level"] = "Strong"
            impact["utility_impact"] = (
                "Moderate - noticeable but manageable accuracy loss"
            )
