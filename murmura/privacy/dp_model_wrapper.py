...
class DPTorchModelWrapper(TorchModelWrapper):
    ...
    def __init__(
        ...
        self.logger.info("DP wrapper initialized - privacy engine will be set up during training")
    ...
    def _make_private(
        ...
        try:
            ...
            try:
                ...
                self.logger.warning(f"make_private_with_epsilon failed: {epsilon_error}")
                ...
                self.logger.info("Successfully made model private with manual parameters")
            
            self.is_dp_enabled = True
            
            # Log privacy parameters
            actual_noise = self.privacy_engine.noise_multiplier
            self.logger.info(f"DP enabled with actual noise multiplier: {actual_noise:.3f}")
            ...
            for epoch in range(epochs):
                ...
                if self.is_dp_enabled:
                    ...
                    if self.dp_config.strict_privacy_check:
                        if self.dp_config.is_privacy_exhausted(self.privacy_spent["epsilon"]):
                            self.logger.warning(
    ...
    def _update_privacy_spent(self) -> None:
        ...
        if self.is_dp_enabled and self.privacy_engine:
            try:
                epsilon = self.privacy_engine.get_epsilon(delta=self.dp_config.target_delta)
                self.privacy_spent = {
                    "epsilon": epsilon,
