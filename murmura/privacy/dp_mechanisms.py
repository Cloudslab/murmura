...
from murmura.privacy.dp_config import DifferentialPrivacyConfig, ClippingStrategy, DPMechanism
...
class ParameterClipper:
    ...
    def __init__(self, strategy: ClippingStrategy, initial_norm: float = 1.0,
                 target_quantile: float = 0.5, adaptation_rate: float = 0.01):
    ...
    def clip_parameters(self, parameters: Dict[str, Any]) -> ClippingResult:
        ...
        if total_norm <= clipping_norm:
            ...
            self.logger.debug(f"Clipped parameters: norm {total_norm:.4f} -> {clipping_norm:.4f} "
    ...
    def _get_clipping_threshold(self, current_norm: float) -> float:
        ...
        if self.strategy == ClippingStrategy.FIXED:
            ...
            if len(self.norm_history) > 10:  # Need some history
                ...
                self.current_norm += self.adaptation_rate * (target_norm - self.current_norm)
                self.current_norm = max(0.1, self.current_norm)  # Minimum threshold

                self.logger.debug(f"Adaptive clipping: norm updated to {self.current_norm:.4f} "
            ...
            if len(self.norm_history) >= 50:  # Need sufficient history
                ...
                self.logger.debug(f"Quantile clipping: {self.target_quantile}-quantile = {self.current_norm:.4f}")
    ...
    def get_clipping_stats(self) -> Dict[str, Any]:
        ...
        recent_norms = self.norm_history[-100:] if len(self.norm_history) >= 100 else self.norm_history
        ...
        return {
            ...
            "clipping_frequency": sum(1 for norm in recent_norms if norm > self.current_norm) / len(recent_norms)
        }
...
class BaseDPMechanism(ABC):
    ...
    @abstractmethod
    def add_noise(self, parameters: Dict[str, Any],
                  sensitivity: float = 1.0) -> Dict[str, Any]:
        """Add calibrated noise to parameters."""
        pass

    @abstractmethod
    def get_privacy_spent(self, num_queries: int = 1,
...
class GaussianMechanism(BaseDPMechanism):
    ...
    def add_noise(self, parameters: Dict[str, Any],
        ...
        self.logger.debug(f"Added Gaussian noise: scale={noise_scale:.4f}, "
    ...
    def get_privacy_spent(self, num_queries: int = 1,
        ...
        if sampling_rate is not None:
            ...
            effective_noise_multiplier = self.noise_multiplier / math.sqrt(amplification_factor)
        else:
...
class LaplaceMechanism(BaseDPMechanism):
    ...
    def __init__(self, config: DifferentialPrivacyConfig):
        ...
        if config.delta is not None:
            raise ValueError("LaplaceMechanism provides pure ε-DP and should not use delta")
...
class DiscreteGaussianMechanism(BaseDPMechanism):
    ...
    def __init__(self, config: DifferentialPrivacyConfig):
        ...
        if config.mechanism != DPMechanism.DISCRETE_GAUSSIAN:
            raise ValueError("DiscreteGaussianMechanism requires DISCRETE_GAUSSIAN mechanism in config")
    ...
    def add_noise(self, parameters: Dict[str, Any],
        ...
        if self.clipping_norm is None:
            raise ValueError("clipping_norm must be set for discrete Gaussian mechanism")
...
class DifferentialPrivacyManager:
    ...
    def apply_differential_privacy(self, parameters_list: List[Dict[str, Any]],
                                   round_number: int,
                                   sampling_rate: Optional[float] = None) -> Tuple[List[Dict[str, Any]], Dict[str, Any]]:
        """
        ...
        if self.config.is_client_side():
            ...
            self.logger.debug(f"Central DP: clipped {len(processed_params)} parameter sets")
        ...
        if self.privacy_accountant is not None:
            ...
            try:
                ...
                self.total_privacy_spent = self.privacy_accountant.get_current_privacy_spent()
    ...
    def add_noise_to_aggregated_parameters(self, aggregated_params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if not self.config.is_central_dp():
            self.logger.warning("add_noise_to_aggregated_parameters called but not using central DP")
