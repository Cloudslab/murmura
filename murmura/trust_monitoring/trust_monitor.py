...
class TrustMonitor:
    ...
    def _compute_parameter_stats(self, parameters: Dict[str, Any]) -> Dict[str, float]:
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Computing parameter stats for {len(parameters)} layers")
        ...
        for layer_name, param_tensor in parameters.items():
            ...
            if np.any(np.isnan(param_array)) or np.any(np.isinf(param_array)):
                self.logger.warning(f"Found NaN/inf values in layer {layer_name}, replacing with zeros")
            ...
            if np.isnan(layer_mean) or np.isnan(layer_std) or np.isnan(layer_l2_norm):
                self.logger.warning(f"Layer {layer_name} produced NaN statistics, using defaults")
                layer_mean = 0.0
                layer_std = 1.0
                layer_l2_norm = 1.0
            ...
            self.logger.info(f"TRUST MONITOR {self.node_id}: Layer {layer_name}: mean={layer_mean:.6f}, std={layer_std:.6f}, l2_norm={layer_l2_norm:.6f}, size={len(param_array)}")
        
        # Global statistics with NaN protection
        all_params_array = np.array(all_params)
        all_params_array = np.nan_to_num(all_params_array, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        # Final NaN check
        if np.isnan(global_mean) or np.isnan(global_std) or np.isnan(global_l2_norm) or np.isnan(global_magnitude):
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Global stats: mean={global_mean:.6f}, std={global_std:.6f}, l2_norm={global_l2_norm:.6f}, magnitude={global_magnitude:.6f}, total_params={len(all_params_array)}")
        
        return param_stats
        
    def _compute_update_similarity(self, update1: ParameterUpdate, update2: ParameterUpdate) -> Dict[str, float]:
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Similarity between {update1.neighbor_id} and {update2.neighbor_id}:")
        self.logger.info(f"TRUST MONITOR {self.node_id}:   L2 norms: {stats1['global_l2_norm']:.6f} vs {stats2['global_l2_norm']:.6f}, ratio: {l2_norm_ratio:.6f}")
        self.logger.info(f"TRUST MONITOR {self.node_id}:   Magnitudes: {stats1['global_magnitude']:.6f} vs {stats2['global_magnitude']:.6f}, ratio: {magnitude_ratio:.6f}")
        self.logger.info(f"TRUST MONITOR {self.node_id}:   Std devs: {stats1['global_std']:.6f} vs {stats2['global_std']:.6f}, ratio: {std_ratio:.6f}")
        ...
        try:
            ...
            if params1_list and params2_list:
                ...
                if np.isnan(cosine_sim):
                    self.logger.warning("Cosine similarity produced NaN, using default value")
                    cosine_sim = 0.0
                
                similarities["cosine_similarity"] = float(cosine_sim)
                self.logger.info(f"TRUST MONITOR {self.node_id}:   Cosine similarity: {cosine_sim:.6f}")
            else:
                similarities["cosine_similarity"] = 0.0
                self.logger.info(f"TRUST MONITOR {self.node_id}:   Cosine similarity: 0.0 (no common parameters)")
    ...
    def _detect_anomalies_cusum(self, neighbor_id: str, current_update: ParameterUpdate) -> Tuple[bool, float, Dict[str, Any]]:
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: CUSUM detection for {neighbor_id}: history length = {history_length}")
        
        # Need at least 1 historical update to compare against
        if history_length < 1:
            self.logger.debug(f"Insufficient history for {neighbor_id} (need 1, have {history_length})")
            return False, 0.0, {}
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Computing similarities for {neighbor_id} with last {num_comparisons} historical updates")
        ...
        for i, past_update in enumerate(comparison_history):
            ...
            combined_sim = (
                sim_metrics["cosine_similarity"] * 0.6 +  # Increased weight for cosine similarity
            ...
            self.logger.info(f"TRUST MONITOR {self.node_id}:   Similarity {i}: cosine={sim_metrics['cosine_similarity']:.6f}, l2_ratio={sim_metrics['l2_norm_ratio']:.6f}, mag_ratio={sim_metrics['magnitude_ratio']:.6f}, combined={combined_sim:.6f}")
        ...
        if len(similarities) >= 1:
            ...
            if np.isnan(mean_sim) or np.isnan(std_sim) or np.isnan(mean_cosine):
                self.logger.warning(f"CUSUM analysis for {neighbor_id}: NaN values detected, skipping")
                return False, 0.0, {"detection_method": "cusum_similarity", "error": "NaN values"}
            ...
            if np.isnan(current_sim) or np.isnan(current_cosine):
                self.logger.warning(f"CUSUM analysis for {neighbor_id}: current similarity is NaN, skipping")
                return False, 0.0, {"detection_method": "cusum_similarity", "error": "NaN current similarity"}
            ...
            cosine_threshold = 0.7  # Cosine similarity threshold for gradient manipulation
            ...
            low_cosine_anomaly = (current_cosine < cosine_threshold) or (cosine_drop > 0.3)
            ...
            is_anomaly = low_cosine_anomaly or (statistical_anomaly and (combined_anomaly or current_cosine < 0.5))
            
            # Calculate confidence score based on multiple factors
            cosine_confidence = max(0.0, (cosine_threshold - current_cosine) / cosine_threshold) if current_cosine < cosine_threshold else 0.0
            statistical_confidence = min(float(abs_z_score) / z_threshold, 2.0) if abs_z_score > z_threshold else 0.0
            combined_confidence = max(0.0, (combined_threshold - current_sim) / combined_threshold) if current_sim < combined_threshold else 0.0
            
            anomaly_score = float(max(cosine_confidence, statistical_confidence * 0.7, combined_confidence * 0.5))
            
            self.logger.info(f"TRUST MONITOR {self.node_id}: CUSUM analysis for {neighbor_id}:")
            self.logger.info(f"  mean_sim={mean_sim:.6f}, std_sim={std_sim:.6f}, current_sim={current_sim:.6f}")
            self.logger.info(f"  mean_cosine={mean_cosine:.6f}, current_cosine={current_cosine:.6f}")
            self.logger.info(f"  z_score={z_score:.6f}, abs_z_score={abs_z_score:.6f}")
            self.logger.info(f"  low_cosine_anomaly={low_cosine_anomaly}, statistical_anomaly={statistical_anomaly}, combined_anomaly={combined_anomaly}")
            self.logger.info(f"  is_anomaly={is_anomaly}, anomaly_score={anomaly_score:.6f}")
    ...
    def _detect_consensus_violations(self, round_num: int, all_updates: Dict[str, ParameterUpdate]) -> Dict[str, Tuple[bool, float, Dict[str, Any]]]:
        """Detect neighbors that violate consensus with majority."""
        violations: Dict[str, Tuple[bool, float, Dict[str, Any]]] = {}
        
        self.logger.info(f"TRUST MONITOR {self.node_id}: Consensus detection with {len(all_updates)} updates (min required: {self.config.min_neighbors_for_consensus})")
        ...
        similarity_matrix = {}
        self.logger.info(f"TRUST MONITOR {self.node_id}: Computing pairwise similarities for {len(neighbor_ids)} neighbors")
        for i, neighbor1 in enumerate(neighbor_ids):
            for j, neighbor2 in enumerate(neighbor_ids):
                if i != j:
                    ...
                    similarity_matrix[(neighbor1, neighbor2)] = sim_metrics["cosine_similarity"]
        ...
        for neighbor in neighbor_ids:
            similarities_to_others = [
                ...
                for other in neighbor_ids if other != neighbor
            ]
            consensus_scores[neighbor] = np.mean(similarities_to_others) if similarities_to_others else 0.0
            self.logger.info(f"TRUST MONITOR {self.node_id}: Consensus score for {neighbor}: {consensus_scores[neighbor]:.6f} (from {len(similarities_to_others)} comparisons)")
        ...
        if len(consensus_scores) >= 2:
            ...
            self.logger.info(f"TRUST MONITOR {self.node_id}: Consensus outlier detection: Q1={q1:.6f}, Q3={q3:.6f}, IQR={iqr:.6f}, lower_bound={lower_bound:.6f}, upper_bound={upper_bound:.6f}")
            
            for neighbor, score in consensus_scores.items():
                # Check for both low and high outliers
                is_low_outlier = score < lower_bound and iqr > 0.01  # More sensitive threshold
                is_high_outlier = score > upper_bound and iqr > 0.01  # Also check for suspicious consistency
                ...
                self.logger.info(f"TRUST MONITOR {self.node_id}: Neighbor {neighbor}: consensus_score={score:.6f}, is_low_outlier={is_low_outlier}, is_high_outlier={is_high_outlier}, violation_score={violation_score:.6f}")
    ...
    def update_trust_scores(self, anomalies: Dict[str, Tuple[bool, float, Dict[str, Any]]], round_num: int) -> None:
        ...
        for neighbor_id in self.trust_scores.keys():
            if neighbor_id in anomalies:
                ...
                if is_anomaly:
                    ...
                    # Recover trust score slightly
                    old_score = self.trust_scores[neighbor_id]
                    self.trust_scores[neighbor_id] = min(
                        1.0, self.trust_scores[neighbor_id] * self.config.trust_recovery_factor
                    )
                    score_changes[neighbor_id] = self.trust_scores[neighbor_id] - old_score
            else:
    ...
    def process_parameter_updates(self, round_num: int, neighbor_updates: Dict[str, Dict[str, Any]], 
                                neighbor_losses: Optional[Dict[str, float]] = None) -> Dict[str, float]:
        """
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: ========== ROUND {round_num} ===========")
        self.logger.info(f"TRUST MONITOR {self.node_id}: Processing {len(neighbor_updates)} neighbors: {list(neighbor_updates.keys())}")
        
        # Log detailed parameter information for each neighbor
        for neighbor_id, parameters in neighbor_updates.items():
            if parameters:
                param_sizes = {name: np.array(param).size for name, param in parameters.items()}
                total_params = sum(param_sizes.values())
                self.logger.info(f"TRUST MONITOR {self.node_id}: Neighbor {neighbor_id} - {len(parameters)} layers, {total_params} total parameters")
                ...
                self.logger.info(f"TRUST MONITOR {self.node_id}: Neighbor {neighbor_id} sample values from {sample_layer}: {sample_values}")
            else:
                self.logger.warning(f"TRUST MONITOR {self.node_id}: Neighbor {neighbor_id} has EMPTY parameters!")
        ...
        for neighbor_id, parameters in neighbor_updates.items():
            ...
            reported_loss = neighbor_losses.get(neighbor_id) if neighbor_losses else None
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Running CUSUM detection on {len(current_updates)} neighbors")
        
        # 1. CUSUM-based detection for each neighbor
        for neighbor_id, update in current_updates.items():
            is_anomaly, score, evidence = self._detect_anomalies_cusum(neighbor_id, update)
            self.logger.info(f"TRUST MONITOR {self.node_id}: CUSUM {neighbor_id} - anomaly={is_anomaly}, score={score:.3f}")
            if is_anomaly:
        ...
        consensus_violations = self._detect_consensus_violations(round_num, current_updates)
        for neighbor_id, (is_violation, score, evidence) in consensus_violations.items():
            self.logger.info(f"TRUST MONITOR {self.node_id}: Consensus {neighbor_id} - violation={is_violation}, score={score:.3f}")
            if is_violation:
        ...
        self.logger.info(f"TRUST MONITOR {self.node_id}: Total anomalies detected: {len(all_anomalies)}")
        ...
        relative_threshold = self.config.suspicion_threshold  # Fallback to absolute threshold
        ...
        if len(self.trust_scores) > 1:
            ...
            if suspicious_neighbors:
                ...
                self.logger.info(f"TRUST MONITOR {self.node_id}: No suspicious neighbors using continuous threshold")
        else:
            ...
            if suspicious_neighbors:
                ...
                self.logger.info(f"TRUST MONITOR {self.node_id}: No suspicious neighbors detected (single neighbor case)")
    ...
    def _calculate_continuous_relative_threshold(self, scores: List[float]) -> float:
        ...
        relative_threshold = (1 - iqr_factor) * q1_based_threshold + iqr_factor * median_based_threshold
        
        self.logger.info(f"TRUST MONITOR {self.node_id}: Continuous threshold calculation:")
        self.logger.info(f"  Q1={q1:.6f}, Q3={q3:.6f}, IQR={iqr:.6f}, Median={median_score:.6f}")
