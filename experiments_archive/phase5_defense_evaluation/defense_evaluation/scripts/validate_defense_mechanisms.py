...
def validate_structural_noise_injection():
    ...
    test_data = {"communications": test_comm_data, "parameter_updates": test_param_data}
    ...
    for noise_level in ["weak", "medium", "strong"]:
        ...
        if defended_comm_count > original_comm_count:
            logger.info(
                f"  ✓ Dummy communications added: {original_comm_count} -> {defended_comm_count}"
            )
        else:
            logger.info(
                f"  ✓ No dummy communications added (expected for small dataset): {original_comm_count}"
            )

        # Validate timing was modified
        original_timestamps = test_data["communications"]["timestamp"].values
        defended_timestamps = defended_data["communications"]["timestamp"].values[
...
def validate_topology_aware_dp():
    ...
    test_data = {"topology": test_topology_data, "parameter_updates": test_param_data}
...
def validate_dynamic_topology_reconfiguration():
    ...
    test_data = {"topology": test_topology_data, "communications": test_comm_data}
    ...
    assert len(defended_topology) == len(original_topology), (
        "Topology size changed unexpectedly"
    )
...
def validate_attacks_produce_different_results():
    ...
    data_dir = Path(
        "/Users/MRANGWALA/Documents/Projects/PhD-Projects/murmura/experiments_archive/phase1_baseline_analysis/training_data"
    )
    exp_dirs = [
        d
        for d in data_dir.iterdir()
        if d.is_dir() and "star" in d.name and "mnist" in d.name
    ]
    ...
    for attack in attacks:
        ...
        logger.info(
            f"  Original {attack.name}: {result.get('attack_success_metric', 0.0):.3f}"
        )
    ...
    for attack in attacks:
        ...
        logger.info(
            f"  Defended {attack.name}: {result.get('attack_success_metric', 0.0):.3f}"
        )

    # Validate that results are different (not necessarily better, but different)
    differences = [
        abs(orig - def_) for orig, def_ in zip(original_results, defended_results)
    ]

    # At least one attack should show a measurable difference (>1%)
    assert any(diff > 0.01 for diff in differences), (
        "No meaningful differences in attack results"
    )
