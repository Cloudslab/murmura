...
def run_attacks_on_data(data: dict) -> dict:
    ...
    for attack in attacks:
        try:
            ...
            print(
                f"{attack.name}: Success = {result.get('attack_success_metric', 0):.3f}"
            )
        except Exception as e:
...
def test_defenses():
    ...
    def print_attack_comparison(original, defended, defense_name):
        print(f"\n{defense_name}:")
        for i, (orig_attack, def_attack) in enumerate(
            zip(original["attack_results"], defended["attack_results"])
        ):
    ...
    print_attack_comparison(
        original_results, noise_results, "Structural Noise Injection"
    )
    print_attack_comparison(original_results, dp_results, "Topology-Aware DP")

    # Save results to file
    output_dir = Path(
        "/Users/MRANGWALA/Documents/Projects/PhD-Projects/murmura/defense_test_results"
    )
