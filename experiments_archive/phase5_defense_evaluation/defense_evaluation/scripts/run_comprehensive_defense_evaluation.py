...
class ComprehensiveDefenseTest:
    ...
    def run_comprehensive_test(
        self, max_experiments: int = 15, filter_pattern: str = None
    ) -> Dict[str, Any]:
        ...
        print(
            f"Testing {len(experiment_dirs)} experiments with {len(self.defense_configs)} defenses"
        )
    ...
    def _update_summary_stats(
        ...
        for defense_name, defense_attack_results in defense_results.items():
            ...
            for orig_attack, def_attack in zip(
                original_results, defense_attack_results["attack_results"]
            ):
    ...
    def _save_results(self, results: Dict[str, Any]):
        ...
        print(
            f"Results saved to: {self.output_dir / 'comprehensive_defense_results.json'}"
        )
    ...
    def _generate_visualizations(self, results: Dict[str, Any]):
        ...
        if defense_names:
            ...
            plt.tight_layout()
            plt.savefig(
                self.output_dir / "defense_effectiveness.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
        ...
        if attack_reductions:
            ...
            for defense_name in defense_names:
                row_data = []
                for attack_full_name in [
                    "Communication Pattern Attack",
                    "Parameter Magnitude Attack",
                    "Topology Structure Attack",
                ]:
                    attack_stats = attack_reductions[defense_name].get(
                        attack_full_name, {}
                    )
            ...
            if heatmap_data:
                ...
                plt.tight_layout()
                plt.savefig(
                    self.output_dir / "defense_heatmap.png",
                    dpi=300,
                    bbox_inches="tight",
                )
                plt.close()
...
def main():
    ...
    results = tester.run_comprehensive_test(
        max_experiments=15, filter_pattern="mnist_federated_star"
    )
    ...
    if defense_effectiveness:
        ...
        for i, (defense_name, effectiveness) in enumerate(sorted_defenses, 1):
            print(
                f"{i}. {defense_name.replace('_', ' ').title()}: {effectiveness:.1f}% avg reduction"
            )
