...
class GranularDefenseAnalyzer:
    ...
    def analyze_by_topology(self):
        ...
        for result in self.detailed_results:
            ...
            for attack_name, reduction_info in effectiveness[
                "attack_reductions"
            ].items():
                if (
                    attack_name
                    in topology_analysis[topology][defense_name]["attack_reductions"]
                ):
                    topology_analysis[topology][defense_name]["attack_reductions"][
                        attack_name
                    ].append(reduction_info["reduction_percentage"])
            ...
            for defense_name in reconfig_analysis[topology]:
                ...
                if data["effectiveness_scores"]:
                    data["overall_stats"] = {
                        ...
                        "non_zero_count": sum(
                            1 for x in data["effectiveness_scores"] if x > 0
                        ),
                    }
                ...
                if data["topology_structure_reductions"]:
                    data["topology_attack_stats"] = {
                        ...
                        "non_zero_count": sum(
                            1 for x in data["topology_structure_reductions"] if x > 0
                        ),
                    }
    ...
    def analyze_by_dataset(self):
        ...
        for result in self.detailed_results:
            ...
            for attack_name, reduction_info in effectiveness[
                ...
                if (
                    attack_name
                    in dataset_analysis[dataset][defense_name]["attack_reductions"]
                ):
                    dataset_analysis[dataset][defense_name]["attack_reductions"][
                        attack_name
                    ].append(reduction_info["reduction_percentage"])
    ...
    def create_granular_report(self, output_file: str):
        ...
        report = {
            ...
            "summary_insights": self.generate_insights(
                topology_analysis, reconfig_analysis, dataset_analysis
            ),
        }
        ...
        for topology in reconfig_analysis:
            ...
            for defense in reconfig_analysis[topology]:
                ...
                topology_insights[defense] = {
                    ...
                    "success_rate": stats.get("non_zero_count", 0)
                    / max(stats.get("count", 1), 1)
                    * 100,
                }
            insights["dynamic_reconfiguration_insights"][
                "topology_specific_performance"
            ][topology] = topology_insights
    ...
    def create_visualization_dashboard(self, output_dir: str):
        ...
        self.plot_topology_effectiveness_heatmap(
            topology_analysis, output_path / "topology_effectiveness_heatmap.png"
        )

        # 2. Attack-specific performance breakdown
        self.plot_attack_specific_breakdown(
            topology_analysis, output_path / "attack_specific_breakdown.png"
        )

        # 3. Dynamic reconfiguration analysis
        reconfig_analysis = self.analyze_dynamic_reconfig_by_topology()
        self.plot_dynamic_reconfig_analysis(
            reconfig_analysis, output_path / "dynamic_reconfig_analysis.png"
        )
    ...
    def plot_topology_effectiveness_heatmap(self, topology_analysis, output_file):
        ...
        for i, defense in enumerate(defenses):
            for j, topology in enumerate(topologies):
                if defense in topology_analysis[topology]:
                    stats = topology_analysis[topology][defense].get(
                        "overall_stats", {}
                    )
        ...
        plt.title(
            "Defense Effectiveness by Network Topology", fontsize=16, fontweight="bold"
        )
    ...
    def plot_attack_specific_breakdown(self, topology_analysis, output_file):
        ...
        for idx, attack_type in enumerate(attack_types):
            ...
            for defense in defense_names:
                ...
                for topology in topology_analysis:
                    if defense in topology_analysis[topology]:
                        attack_data = topology_analysis[topology][defense][
                            "attack_reductions"
                        ].get(attack_type, {})
            ...
            ax.set_xticklabels(
                [name.replace("_", " ").title() for name in defense_names],
                rotation=45,
                ha="right",
            )
        ...
        plt.suptitle(
            "Defense Effectiveness by Attack Type", fontsize=16, fontweight="bold"
        )
        plt.tight_layout()
        plt.savefig(output_file, dpi=300, bbox_inches="tight")
        plt.close()
    ...
    def plot_dynamic_reconfig_analysis(self, reconfig_analysis, output_file):
        ...
        defense_types = [
            "topology_reconfig_weak",
            "topology_reconfig_medium",
            "topology_reconfig_strong",
        ]
        ...
        for i, defense in enumerate(defense_types):
            for j, topology in enumerate(topologies):
                if defense in reconfig_analysis[topology]:
                    stats = reconfig_analysis[topology][defense].get(
                        "overall_stats", {}
                    )
        ...
        sns.heatmap(
            ...
            xticklabels=topologies,
            yticklabels=[
                d.replace("topology_reconfig_", "").title() for d in defense_types
            ],
            annot=True,
            fmt=".1f",
            cmap="RdYlGn",
        ...
        ax1.set_title(
            "Dynamic Reconfiguration Success Rate by Topology", fontweight="bold"
        )
...
def main():
    ...
    for topology, performance in reconfig_insights[
        "topology_specific_performance"
    ].items():
        ...
        for defense, stats in performance.items():
            ...
            print(
                f"     {defense}: {success_rate:.1f}% success rate, {mean_eff:.2f}% avg effectiveness"
            )
