...
class ComprehensivePhase1Evaluator:
    ...
    def __init__(self, data_dir: str, output_dir: str, max_workers: int = 4):
        ...
        self.defense_configs = {
            ...
            "structural_noise_weak": DefenseConfig(
                ...
                magnitude_noise_multiplier=0.05,
            ...
            "structural_noise_medium": DefenseConfig(
                ...
                enable_magnitude_noise=True,
                magnitude_noise_multiplier=0.15,
                enable_topology_reconfig=False,
                enable_topology_aware_dp=False,
            ),
            "structural_noise_strong": DefenseConfig(
                ...
                enable_magnitude_noise=True,
                magnitude_noise_multiplier=0.3,
                enable_topology_reconfig=False,
                enable_topology_aware_dp=False,
            ),
            ...
            "combined_strong": DefenseConfig(
                ...
                preserve_connectivity=True,
                enable_topology_aware_dp=True,
                structural_amplification_factor=1.8,
                neighbor_correlation_weight=0.15,
            ),
        }

        self.logger.info(
            f"Initialized evaluator with {len(self.defense_configs)} defense configurations"
        )
        ...
        for data_type, filename in data_files.items():
            ...
            if filepath.exists():
                try:
                    ...
                    self.logger.warning(
                        f"Failed to load {filename} from {experiment_dir.name}: {e}"
                    )
    ...
    def apply_defense(
        self, data: Dict[str, pd.DataFrame], config: DefenseConfig
    ...
    def evaluate_single_experiment(
        self, experiment_path: Tuple[Path, str]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                len(original_data) < 3
            ):  # Need at least communications, parameters, topology
                return {
            ...
            effectiveness_metrics = self._calculate_effectiveness(
                original_results["attack_results"], defended_results["attack_results"]
            )
            ...
            self.logger.error(
                f"Failed to evaluate {experiment_name} with {defense_name}: {e}"
            )
            return {
                "experiment_name": experiment_name,
                "defense_name": defense_name,
    ...
    def _calculate_effectiveness(
        self, original_attacks: List[Dict], defended_attacks: List[Dict]
    ) -> Dict[str, Any]:
        ...
        for orig_attack, def_attack in zip(original_attacks, defended_attacks):
            ...
            if orig_success > 0:
                reduction_pct = (
                    max(0.0, (orig_success - def_success) / orig_success) * 100
                )
        ...
        if metrics["valid_attacks"] > 0:
            metrics["overall_effectiveness"] = (
                total_reduction / metrics["valid_attacks"]
            )

        return metrics

    def run_comprehensive_evaluation(
        self, max_experiments: int = None, dataset_filter: str = None
    ) -> Dict[str, Any]:
        ...
        if dataset_filter:
            ...
            self.logger.info(
                f"Filtered to {dataset_filter} experiments: {len(experiment_dirs)}"
            )
        ...
        for batch_idx in range(num_batches):
            ...
            with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
                futures = [
                    executor.submit(self.evaluate_single_experiment, pair)
            ...
            if (batch_idx + 1) % 5 == 0:  # Save every 5 batches
                self._save_intermediate_results(
                    all_results, summary_stats, batch_idx + 1
                )
        ...
        self.logger.info(
            f"Successful evaluations: {summary_stats['successful_evaluations']}"
        )
    ...
    def _finalize_summary_stats(self, summary_stats: Dict[str, Any]):
        ...
        for defense_name, attack_reductions in summary_stats[
            "attack_reductions"
        ].items():
    ...
    def _save_intermediate_results(
        self, results: List[Dict], summary_stats: Dict, batch_num: int
    ):
        """Save intermediate results after each batch."""
        intermediate_file = (
            self.output_dir / f"intermediate_results_batch_{batch_num}.json"
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Comprehensive Phase1 Defense Evaluation"
    )
    parser.add_argument(
        "--data-dir",
        type=str,
    ...
    evaluator = ComprehensivePhase1Evaluator(
        args.data_dir, args.output_dir, max_workers=args.max_workers
    )

    # Run comprehensive evaluation
    results = evaluator.run_comprehensive_evaluation(
        max_experiments=args.max_experiments, dataset_filter=args.dataset_filter
    )
    ...
    print(
        f"Success rate: {results['summary_stats']['successful_evaluations'] / results['summary_stats']['total_evaluations'] * 100:.1f}%"
    )
