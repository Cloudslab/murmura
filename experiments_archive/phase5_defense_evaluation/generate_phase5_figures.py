...
class Phase5FigureGenerator:
    ...
    def __init__(self, data_dir: str, output_dir: str):
        ...
        self.fig_params = {"dpi": 300, "facecolor": "white", "edgecolor": "none"}
    ...
    def create_defense_effectiveness_overview(self):
        ...
        for i, attack_name in enumerate(attack_names):
            ...
            for config, results_list in regular_results.get(
                "baseline_results", {}
            ).items():
            ...
            for config, results_list in subsampling_results.get(
                "baseline_results", {}
            ).items():
                for result in results_list:
                    if f"{attack_name} Attack" in result:
            ...
            colors = [
                ...
                self.colors["structural_weak"],
                self.colors["structural_medium"],
                self.colors["structural_strong"],
            ]
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "defense_effectiveness_overview.png", **self.fig_params
        )
        plt.close()
    ...
    def create_attack_effectiveness_breakdown(self):
        ...
        for i, attack_name in enumerate(attack_names):
            ...
            if attack_name in attack_effectiveness:
                ...
                y_pos_subsampling = (
                    np.arange(len(subsampling_configs)) + len(regular_configs) + 1
                )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "attack_effectiveness_breakdown.png", **self.fig_params
        )
        plt.close()
    ...
    def create_topology_effectiveness_analysis(self):
            ...
            if topology in topology_data:
                ...
                for attack_name in attack_types:
                    ...
                    regular_values.append(
                        regular_avg / regular_count if regular_count > 0 else 0
                    )
                    ...
                    for config, results in subsampling_data.items():
                        if attack_name in results:
                            ...
                            subsampling_count += 1
                    subsampling_values.append(
                        subsampling_avg / subsampling_count
                        if subsampling_count > 0
                        else 0
                    )
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "topology_effectiveness_analysis.png", **self.fig_params
        )
        plt.close()
    ...
    def create_statistical_significance_plots(self):
        ...
        for i, attack_name in enumerate(attack_names):
            ...
            if baseline_values and defended_values:
                ...
                ax.text(
                    1.5,
                    max(max(baseline_values), max(defended_values)) * 0.9,
        ...
        plt.tight_layout()
        plt.savefig(
            self.output_dir / "statistical_significance_plots.png", **self.fig_params
        )
        plt.close()
    ...
    def create_deployment_recommendations(self):
        ...
        # Create heatmap
        im = ax.imshow(
            effectiveness_matrix, cmap="RdYlGn", aspect="auto", vmin=0, vmax=0.5
        )
        ...
        cbar.set_label(
            "Additional Attack Reduction", rotation=270, labelpad=20, fontweight="bold"
        )

        plt.tight_layout()
        plt.savefig(
            self.output_dir / "deployment_recommendations.png", **self.fig_params
        )
        plt.close()
