...
def analyze_topology_dp_implementation():
    ...
    for attack_type in baseline_dp_results:
        ...
        print(
            f"   {attack_type:30} | {baseline:8.1f}%   | {ours:12.1f}%   | {gap:+6.1f}%"
        )
...
def calculate_proper_dp_noise_levels():
    ...
    for strength, amp in zip(["weak", "medium", "strong"], amplification_factors):
        ...
        print(
            f"     - {strength:6}: σ = {our_noise:.4f} (vs proper DP: {ratio_to_proper_dp:.1%})"
        )

    print("\n3. CONCLUSION:")
    print(f"     Proper DP σ: {sigma_dp:.3f}")
    print(
        f"     Our strongest noise: {parameter_norm * base_noise_multiplier * 2.0:.4f}"
    )
    print(
        f"     We use {(parameter_norm * base_noise_multiplier * 2.0 / sigma_dp * 100):.1f}% of proper DP noise levels!"
    )
