...
def create_defense_effectiveness_flow():
    ...
    results_file = Path(
        "comprehensive_layered_privacy_evaluation/comprehensive_layered_privacy_results.json"
    )
    ...
    for i, attack_type in enumerate(attack_types):
        if attack_type in baseline_values:
            ...
            ax.plot(
                ...
                markersize=10,
                color=colors[i],
                label=attack_type.replace(" Attack", ""),
                alpha=0.8,
            )
    ...
    for i in range(len(defense_levels) - 1):
        ax.annotate(
            ...
            xytext=(i, 0.95),
            arrowprops=dict(
                arrowstyle="->", color=user_color_palette[6], lw=2, alpha=0.7
            ),
        )
    ...
    plt.tight_layout()
    plt.savefig(
        "figures/defense_effectiveness_flow.png",
        dpi=300,
        facecolor="white",
        edgecolor="none",
    )
...
def create_layered_protection_comparison():
    ...
    plt.tight_layout()
    plt.savefig(
        "figures/layered_protection_comparison.png",
        dpi=300,
        facecolor="white",
        edgecolor="none",
    )
