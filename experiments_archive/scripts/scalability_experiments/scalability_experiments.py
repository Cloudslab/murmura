...
def main():
    ...
    if args.quick_test:
        ...
        while current <= args.max_nodes:
            ...
            if current < 100:
                ...
                current += 100
        print(
            f"🚀 Running comprehensive scalability test: {args.min_nodes}-{args.max_nodes} nodes"
        )
    ...
    try:
        ...
        if results["total_experiments"] > 0:
            print(
                f"   Success rate: {results['successful_experiments'] / results['total_experiments']:.1%}"
            )
    ...
    with open(summary_file, "w") as f:
        ...
        if "scaling_trends" in analysis:
            ...
            for topology, trend_data in analysis["scaling_trends"].items():
                ...
                if success_by_size:
                    ...
                    for size, success_rate in sorted(
                        success_by_size.items(), key=lambda x: int(x[0])
                    ):
                        signal = signal_by_size.get(size, 0.0)
                        f.write(
                            f"    {int(size):3d} nodes: {success_rate:.1%} success, {signal:.3f} signal strength\n"
                        )
        ...
        if "topology_analysis" in analysis:
            ...
            sorted_topologies = sorted(
                ranking.items(), key=lambda x: x[1]["attack_success"], reverse=True
            )
            ...
            for i, (topology, metrics) in enumerate(sorted_topologies, 1):
                ...
                f.write(
                    f"{i}. {topology.upper():12s}: {success_rate:.1%} success, {signal_strength:.3f} avg signal\n"
                )
        ...
        if "dp_effectiveness" in analysis and analysis["dp_effectiveness"]:
            ...
            if "without_dp" in dp_data and "with_dp" in dp_data:
                ...
                f.write(
                    f"Without DP: {no_dp.get('attack_success', 0):.1%} attack success\n"
                )
                f.write(
                    f"With DP:    {with_dp.get('attack_success', 0):.1%} attack success\n"
                )
                ...
                if protection:
                    ...
                    f.write(
                        f"DP Protection: {reduction:.1%} reduction in attack effectiveness\n"
                    )
        ...
        if "complexity_bounds" in analysis:
            ...
            f.write(
                f"Tested network size range: {size_range.get('min', 0)}-{size_range.get('max', 0)} nodes\n"
            )
            f.write(
                f"Extrapolation validity: {bounds.get('extrapolation_validity', 'unknown').upper()}\n"
            )
            f.write(
                f"Recommended max extrapolation: {bounds.get('recommended_max_extrapolation', 0)} nodes\n"
            )

            limits = bounds.get("theoretical_limits", {})
            if limits:
                f.write(
                    f"Information-theoretic limit: {limits.get('information_theoretic', 0):.3f}\n"
                )
                f.write(
                    f"Network effect bound: {limits.get('network_effect_bound', 0):.3f}\n"
                )
