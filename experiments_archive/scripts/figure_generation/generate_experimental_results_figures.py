...
def create_baseline_effectiveness_figure():
    ...
    summary = (
        df_filtered.groupby(["topology", "attack_short"])["success_rate"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    ...
    for i, attack in enumerate(attacks):
        ...
        means = [
            attack_data[attack_data["topology"] == t]["mean"].iloc[0]
            if len(attack_data[attack_data["topology"] == t]) > 0
            else 0
            for t in topologies
        ]
        errors = [
            attack_data[attack_data["topology"] == t]["ci"].iloc[0]
            if len(attack_data[attack_data["topology"] == t]) > 0
            else 0
            for t in topologies
        ]
    ...
    ax.set_title(
        "Baseline Attack Effectiveness Across Network Topologies\n(Complete Knowledge, No Differential Privacy)",
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase1_figures/fig1_attack_effectiveness.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase1_figures/fig1_attack_effectiveness.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_realistic_scenarios_figure():
    ...
    user_cmap = LinearSegmentedColormap.from_list(
        "user_palette", [color_3[0], color_3[1], color_3[2]]
    )
    ...
    ax.set_title(
        "Attack Effectiveness Under Realistic Partial Topology Knowledge\n(✓ indicates effectiveness above 30% threshold)",
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase2_figures/fig2_realistic_scenarios.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase2_figures/fig2_realistic_scenarios.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_dp_effectiveness_figure():
    ...
    summary = (
        df_filtered.groupby(["dp_label", "attack_name"])["success_rate"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    summary["ci"] = 1.96 * summary["std"] / np.sqrt(summary["count"])
    ...
    ax.set_ylabel("Attack Success Rate", fontweight="bold")
    ax.set_title(
        "Attack Effectiveness Under Differential Privacy Protection",
        fontweight="bold",
        pad=20,
    )
    ax.set_xticks(x + width)
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase1_figures/fig3_dp_effectiveness.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase1_figures/fig3_dp_effectiveness.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_dataset_comparison_figure():
    ...
    summary = (
        df_filtered.groupby(["dataset", "attack_name"])["success_rate"]
        .agg(["mean", "std", "count"])
        .reset_index()
    )
    summary["ci"] = 1.96 * summary["std"] / np.sqrt(summary["count"])
    ...
    for i, dataset in enumerate(datasets):
        ...
        for _, row in dataset_summary.iterrows():
            ...
            ax.plot(
                [x_pos, x_pos],
                [mean_val - ci_val, mean_val + ci_val],
                color="red",
                linewidth=2,
                alpha=0.8,
                zorder=10,
    ...
    ax.set_title(
        "Attack Effectiveness Across Data Modalities\n(Domain-Agnostic Vulnerability Analysis)",
        fontweight="bold",
        pad=20,
    )
    ax.yaxis.set_major_formatter(plt.FuncFormatter(lambda y, _: "{:.0%}".format(y)))
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase1_figures/fig4_dataset_violin.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase1_figures/fig4_dataset_violin.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_scalability_figure():
    ...
    ax.set_ylabel("Attack Success Rate", fontweight="bold")
    ax.set_title(
        "Scale Independence of Topology-Based Attacks\n(Enterprise Network Analysis: 50-500 Nodes)",
        fontweight="bold",
        pad=20,
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase4_figures/fig5_network_scaling.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase4_figures/fig5_network_scaling.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_summary_results_table():
    ...
    results_data = {
        ...
        "Communication Pattern": ["84.1%", "65.0% avg", "62.3% avg", "68.7% avg"],
        "Parameter Magnitude": ["65.0%", "55.4% avg", "52.8% avg", "55.9% avg"],
        "Topology Structure": ["47.2%", "49.9% avg", "45.1% avg", "48.3% avg"],
        "Key Finding": [
