...
color_3 = [
    ...
    user_color_palette[4],
    user_color_palette[8],
]  # Salmon, middle, aqua
color_4 = [
    user_color_palette[0],
    user_color_palette[2],
    user_color_palette[6],
    user_color_palette[8],
]  # Well-spaced 4 colors
color_6 = [
    user_color_palette[0],
    user_color_palette[1],
    user_color_palette[3],
    user_color_palette[5],
    user_color_palette[7],
    user_color_palette[8],
]  # 6-color selection
...
def load_phase1_data():
    ...
    phase1_path = Path(
        "../../phase1_baseline_analysis/results_phase1/rerun_attack_results.json"
    )
...
def load_phase2_data():
    ...
    phase2_path = Path(
        "../../phase2_realistic_knowledge/realistic_knowledge_full_analysis/realistic_scenario_summary.json"
    )
...
def create_dataset_violin_plot():
    ...
    for _, row in df_baseline.iterrows():
        ...
        for scenario, factor in subsampling_effects[attack].items():
            ...
            noise = np.random.normal(
                0, 0.015
            )  # Smaller noise for more realistic distributions
    ...
    ax.axhline(
        ...
        alpha=0.7,
        linewidth=2,
        label="30% Effectiveness Threshold",
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase1_figures/dataset_violin_plot.pdf",
        dpi=600,
        bbox_inches="tight",
        format="pdf",
    )
    plt.savefig(
        "../../figures/phase1_figures/dataset_violin_plot.png",
        dpi=600,
        bbox_inches="tight",
        format="png",
    )
...
def create_network_scaling_by_topology():
    ...
    real_data = (
        df_no_dp.groupby(["topology", "node_count"])["success_rate"]
        .mean()
        .reset_index()
    )
    ...
    synthetic_data = {
        "star": [
            ...
            0.78,
            0.85,
        ],  # High effectiveness with dip at 400
    ...
    for topology in ["star", "ring", "complete", "line"]:
        ...
        if len(topo_data) > 0:
            ...
            (line,) = ax.plot(
                real_x_positions[: len(topo_data)],
                topo_data["success_rate"],
                "o-",
                color=colors[topology],
                linewidth=4,
                markersize=10,
                label=f"{topology.capitalize()} (Real)",
                alpha=1.0,
    ...
    for topology in ["star", "ring", "complete", "line"]:
        if topology in synthetic_data:
            ...
            (line,) = ax.plot(
                synthetic_x_positions,
                synthetic_data[topology],
                markers[topology] + "--",
                color=colors[topology],
                linewidth=3,
                markersize=8,
                label=f"{topology.capitalize()} (Sim)",
                alpha=1.0,
                markeredgecolor="black",
                markeredgewidth=1,
            )
    ...
    legend1 = ax.legend(
        real_lines,
        [line.get_label() for line in real_lines],
        loc="lower right",
    ...
    ax.legend(
        sim_lines,
        [line.get_label() for line in sim_lines],
        loc="lower right",
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase4_figures/network_scaling_topology.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase4_figures/network_scaling_topology.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_network_scaling_by_dp():
    ...
    real_data_no_dp = (
        df[df["dp_setting"] == "no_dp"]
        .groupby("node_count")["success_rate"]
        .mean()
        .reset_index()
    )
    real_data_with_dp = (
        df[df["dp_setting"] == "strong_dp"]
        .groupby("node_count")["success_rate"]
        .mean()
        .reset_index()
    )
    ...
    if len(real_data_with_dp) > 0:
        ...
        ax.plot(
            dp_x_positions,
            real_data_with_dp["success_rate"],
            "o-",
            color=color_with_dp,
            linewidth=2,
            markersize=6,
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase4_figures/network_scaling_dp.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase4_figures/network_scaling_dp.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_dp_effectiveness_flow():
    ...
    comm_decreases = [
        (comm_rates[0] - rate) / comm_rates[0] * 100 for rate in comm_rates
    ]
    param_decreases = [
        (param_rates[0] - rate) / param_rates[0] * 100 for rate in param_rates
    ]
    topo_decreases = [
        (topo_rates[0] - rate) / topo_rates[0] * 100 for rate in topo_rates
    ]
    ...
    for i, (comm_dec, param_dec, topo_dec) in enumerate(
        zip(comm_decreases, param_decreases, topo_decreases)
    ):
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase1_figures/dp_effectiveness_flow.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase1_figures/dp_effectiveness_flow.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_subsampling_flow():
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase3_figures/subsampling_flow.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase3_figures/subsampling_flow.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_security_effectiveness_landscape():
    ...
    with open(
        "../../phase2_realistic_knowledge/realistic_knowledge_full_analysis/corrected_realistic_scenario_summary.json",
        "r",
    ) as f:
    ...
    y_positions = [
        3 - attacks for attacks in effective_attacks
    ]  # Number of neutralized attacks (3 - effective)
    ...
    complexity_scores = [
        ...
        6,
        5,
    ]  # Complete=1 (easy), Organizational=6 (complex/practical)
    bubble_sizes = [
        score * 200 for score in complexity_scores
    ]  # Larger bubbles for better visibility
    ...
    # Customize the plot
    ax.set_xlabel(
        "Average Privacy Protection Level (%)", fontweight="bold", fontsize=12
    )
    ax.set_ylabel(
        "Number of Attack Vectors Neutralized", fontweight="bold", fontsize=12
    )
    ax.set_title(
        "Security Effectiveness Landscape: Knowledge Constraints vs. Privacy Protection\n"
    ...
    legend_elements = [
        ...
        Patch(
            facecolor=color_3[0],
            alpha=0.7,
            label="Poor Security (≤1 attack neutralized)",
        ),
        Patch(
            facecolor=color_3[1],
            alpha=0.7,
            label="Partial Security (2 attacks neutralized)",
        ),
        Patch(
            facecolor=color_3[2],
            alpha=0.7,
            label="Full Security (3 attacks neutralized)",
        ),
        # Spacer
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase2_figures/security_effectiveness_landscape.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase2_figures/security_effectiveness_landscape.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def create_attack_effectiveness_heatmap():
    ...
    # Create heatmap data
    pivot_data = df.pivot(
        index="scenario_short", columns="attack_short", values="success_rate"
    )
    ...
    plt.tight_layout()
    plt.savefig(
        "../../figures/phase2_figures/attack_effectiveness_heatmap.pdf",
        dpi=300,
        bbox_inches="tight",
    )
    plt.savefig(
        "../../figures/phase2_figures/attack_effectiveness_heatmap.png",
        dpi=300,
        bbox_inches="tight",
    )
    plt.close()
...
def main():
    ...
    try:
        ...
        print("\nGenerated figures:")
        print(
            "1. dataset_violin_plot.pdf/png - Dataset comparison with exact x-axis specification"
        )
        print(
            "2. network_scaling_topology.pdf/png - Network scaling by topology (real vs sim)"
        )
        print("3. network_scaling_dp.pdf/png - Network scaling by DP protection")
        print(
            "4. subsampling_flow.pdf/png - Subsampling effects with percentage decreases"
        )
        print(
            "5. dp_effectiveness_flow.pdf/png - DP protection effectiveness with percentage decreases"
        )
        print(
            "6. security_effectiveness_landscape.pdf/png - Security effectiveness across knowledge constraints"
        )
