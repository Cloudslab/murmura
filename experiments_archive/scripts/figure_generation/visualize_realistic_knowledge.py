...
def create_scenario_comparison_plot(summary: Dict[str, Any], output_dir: str) -> None:
    ...
    for attack_idx, (attack, attack_label) in enumerate(zip(attacks, attack_labels)):
        ...
        for i, scenario in enumerate(scenarios):
            if scenario in summary["scenario_effectiveness"]:
                if attack in summary["scenario_effectiveness"][scenario]:
                    success = summary["scenario_effectiveness"][scenario][attack][
                        "average_success"
                    ]
                    success_rates.append(success)
                    scenario_labels.append(
                        scenario.replace(" (Baseline)", "")
        ...
        # Create bar plot
        bars = ax.bar(
            range(len(success_rates)), success_rates, color=scenario_colors, alpha=0.8
        )
    ...
    plt.tight_layout()
    plt.savefig(
        f"{output_dir}/realistic_scenarios_comparison.pdf", dpi=300, bbox_inches="tight"
    )
    plt.savefig(
        f"{output_dir}/realistic_scenarios_comparison.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def create_degradation_heatmap(summary: Dict[str, Any], output_dir: str) -> None:
    ...
    for scenario in scenarios:
        ...
        for attack in attacks:
            if (
                scenario in summary["scenario_effectiveness"]
                and attack in summary["scenario_effectiveness"][scenario]
            ):
                reduction = summary["scenario_effectiveness"][scenario][attack][
    ...
    sns.heatmap(
        ...
        yticklabels=[
            s.replace("Neighborhood ", "").replace("Organizational ", "Org ")
            for s in scenarios
        ],
        annot=True,
        fmt=".1f",
...
def create_paper_figure(summary: Dict[str, Any], output_dir: str) -> None:
        ...
        for scenario in scenarios:
            if (
                ...
                success_rates.append(
                    summary["scenario_effectiveness"][scenario][attack][
                        "average_success"
                    ]
                )
            else:
    ...
    for scenario in neighborhood_scenarios:
        if (
            scenario in summary["scenario_effectiveness"]
            and "Communication Pattern Attack"
            in summary["scenario_effectiveness"][scenario]
        ):
            comm_success.append(
                summary["scenario_effectiveness"][scenario][
                    "Communication Pattern Attack"
                ]["average_success"]
            )
        else:
    ...
    plt.savefig(
        f"{output_dir}/fig_realistic_knowledge_paper.pdf", dpi=300, bbox_inches="tight"
    )
    plt.savefig(
        f"{output_dir}/fig_realistic_knowledge_paper.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    # Paths
    results_dir = (
        "experiments_archive/results/attack_results/realistic_knowledge_full_analysis"
    )
    ...
    if "Complete Knowledge (Baseline)" in summary["scenario_effectiveness"]:
        for attack, data in summary["scenario_effectiveness"][
            "Complete Knowledge (Baseline)"
        ].items():
        ...
        if scenario in summary["scenario_effectiveness"]:
            for attack in [
                ...
                if attack in summary["scenario_effectiveness"][scenario]:
                    ...
                    reduction = (
                        ((baseline - success) / baseline * 100) if baseline > 0 else 0
                    )
                    effective = "✓" if success > 0.3 else "✗"

                    print(
                        f"  {attack.replace(' Attack', '')}: {success:.2f} ({effective}) (-{reduction:.0f}%)"
                    )
