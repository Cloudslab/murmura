...
class PaperExperimentRunner:
    ...
    def setup_logging(self):
        ...
        log_file = (
            ...
            / "logs"
            / f"paper_experiments_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
        )
    ...
    def get_valid_configurations(
        ...
        all_datasets_and_node_counts = [
            ...
            (
                "ham10000",
                [5, 7, 10, 15, 20, 30],
            ),  # HAM10000: 7 classes, optimal at 7 nodes; using multiples of 10 for cluster efficiency (except 7)
        ]
        ...
        if filters:
            ...
            if filters.get("datasets"):
                all_datasets_and_node_counts = [
                    ...
                    if d in filters["datasets"]
                ]
                self.logger.info(
                    f"Filtering experiments to datasets: {filters['datasets']}"
                )

            # Node count filtering
            if filters.get("node_counts"):
                all_datasets_and_node_counts = [
                    (d, [nc for nc in n if nc in filters["node_counts"]])
                    for d, n in all_datasets_and_node_counts
                ]
                self.logger.info(
                    f"Filtering experiments to node counts: {filters['node_counts']}"
                )
        elif dataset_filter:
            # Legacy dataset_filter support
            if dataset_filter not in ["mnist", "ham10000"]:
                raise ValueError(
                    f"Invalid dataset filter: {dataset_filter}. Must be 'mnist' or 'ham10000'"
                )
            all_datasets_and_node_counts = [
                (d, n) for d, n in all_datasets_and_node_counts if d == dataset_filter
            ]
        ...
        if filters and filters.get("attack_strategies"):
            attack_strategies = [
                a for a in attack_strategies if a in filters["attack_strategies"]
            ]
            self.logger.info(
                f"Filtering experiments to attack strategies: {filters['attack_strategies']}"
            )
        ...
        if filters and filters.get("dp_settings"):
            dp_settings = [
                dp for dp in dp_settings if dp["name"] in filters["dp_settings"]
            ]
            self.logger.info(
                f"Filtering experiments to DP settings: {filters['dp_settings']}"
            )
                ...
                for node_count in node_counts_to_use:
                    for dp_setting in dp_settings:
                        ...
                        if filters and filters.get("fl_types"):
                            fl_types = [
                                fl for fl in fl_types if fl in filters["fl_types"]
                            ]

                        for fl_type in fl_types:
                            valid_topologies = self.compatibility_matrix[fl_type][
        ...
        self.logger.info(
            f"Generated {len(configurations)} valid experimental configurations"
        )
        return configurations

    def get_phase2_sampling_configurations(
        ...
        if filters and filters.get("attack_strategies"):
            focus_attack_strategies = [
                a for a in focus_attack_strategies if a in filters["attack_strategies"]
            ]
        ...
        if filters and filters.get("node_counts"):
            for dataset in focus_node_counts:
                focus_node_counts[dataset] = [
                    n for n in focus_node_counts[dataset] if n in filters["node_counts"]
                ]
        ...
        focus_combinations = [
            (
                "federated",
                "star",
            ),  # Classic FL - most vulnerable to centralized attacks
            ("federated", "complete"),  # Fully connected FL
            (
                "decentralized",
                "ring",
            ),  # Structured decentralized - vulnerable to topology attacks
        ...
        if filters:
            if filters.get("fl_types") or filters.get("topologies"):
                focus_combinations = [
                    (fl, topo)
                    for fl, topo in focus_combinations
                    if (not filters.get("fl_types") or fl in filters["fl_types"])
                    and (not filters.get("topologies") or topo in filters["topologies"])
                ]
        ...
        focus_dp_settings = [
            {"enabled": False, "epsilon": None, "name": "no_dp"},
            {
                ...
                "epsilon": 8.0,
                "name": "medium_dp",
            },  # Most representative DP setting
            {
                "enabled": True,
                "epsilon": 4.0,
                "name": "strong_dp",
            },  # Strong DP to test amplification
        ]
        if filters and filters.get("dp_settings"):
            focus_dp_settings = [
                dp for dp in focus_dp_settings if dp["name"] in filters["dp_settings"]
            ]
        ...
        self.logger.info(
            f"   Sampling scenarios: {[s['name'] for s in sampling_scenarios]}"
        )
            ...
            for attack_strategy in focus_attack_strategies:
                for node_count in focus_node_counts[dataset]:
                    for fl_type, topology in focus_combinations:
                        ...
                        if (
                            topology
                            not in self.compatibility_matrix[fl_type][
                                "compatible_topologies"
                            ]
                        ):
                            continue
                        ...
                        for dp_setting in focus_dp_settings:
                            for sampling_scenario in sampling_scenarios:
                                config = {
                                    ...
                                    "client_sampling_rate": sampling_scenario[
                                        "client_rate"
                                    ],
                                    "data_sampling_rate": sampling_scenario[
                                        "data_rate"
                                    ],
                                    "dataset": dataset,
                                    "attack_strategy": attack_strategy,
        ...
        self.logger.info(
            f"Generated {len(configurations)} Phase 2 sampling configurations"
        )
        return configurations
    ...
    def _estimate_runtime(self, dataset: str, node_count: int, fl_type: str) -> int:
        ...
        base_time = {
            ...
            "ham10000": {
                "federated": 120,
                "decentralized": 180,
            },  # HAM10000 is lightweight medical imaging dataset
        }
    ...
    def run_single_experiment(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"🎯 Starting experiment {self.experiment_id}: {experiment_name}"
        )
        ...
        try:
            ...
            has_success = any(
                indicator in stdout_output for indicator in success_indicators
            )
            has_error = any(
                indicator in stdout_output or indicator in stderr_output
            ...
            if result.returncode != 0 or (has_error and not has_success):
                ...
                if "Traceback" in all_output:
                    ...
                    if traceback_start >= 0:
                        traceback_lines = lines[
                            traceback_start : traceback_start + 20
                ...
                return self._create_failed_result(
                    config, experiment_name, error_msg, start_time
                )
            ...
            viz_dir = (
                self.base_output_dir
                / f"visualizations_phase{self.phase}"
                / experiment_name
            )
            if viz_dir.exists():
                ...
                self.logger.info(
                    f"   📊 Attack success: {'✅' if evaluation['attack_success'] else '❌'} "
                    f"(confidence: {evaluation['confidence_score']:.3f})"
                )

                return self._create_success_result(
                    config, experiment_name, attack_results, evaluation, start_time
                )
            else:
    ...
    def _build_command(
        self, script: str, config: Dict[str, Any], experiment_name: str
    ) -> List[str]:
        """Build command for running experiment."""

        cmd = [
            "python",
            f"murmura/examples/{script}",
            "--partition_strategy",
            ...
            "--rounds",
            "3",  # Keep training fast for large-scale experiments
            "--epochs",
            "1",
            "--vis_dir",
            str(self.base_output_dir / f"visualizations_phase{self.phase}"),
            "--create_summary",  # Enable visualization generation
            "--experiment_name",
            experiment_name,  # Use custom experiment name
        ]
    ...
    def evaluate_attack_comprehensive(
        self, attack_results: Dict[str, Any], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        evaluation["topology_specific_metrics"] = self._analyze_topology_effects(
            attack_results, config
        )

        # Add DP effectiveness metrics
        evaluation["dp_effectiveness_metrics"] = self._analyze_dp_effectiveness(
            attack_results, config
        )

        # Add scalability metrics
        evaluation["scalability_metrics"] = self._analyze_scalability_effects(
            attack_results, config
        )

        return evaluation

    def _analyze_attack_strategy(
        self, attack_results: Dict[str, Any], config: Dict[str, Any]
    ) -> Dict[str, Any]:
            ...
            for result in attack_results.get("attack_results", []):
                if result.get("attack_name") == "Topology Structure Attack":
                    if "correlations" in result:
                        ...
                        max_correlation = max(
                            [abs(v) for v in correlations.values()], default=0.0
                        )
    ...
    def _analyze_topology_effects(
        ...
        metrics = {
            ...
            "communication_complexity": self._calculate_communication_complexity(
                topology, node_count
            ),
        }
    ...
    def _calculate_communication_complexity(
        self, topology: str, node_count: int
    ) -> Dict[str, int]:
    ...
    def _analyze_dp_effectiveness(
        ...
        if dp_setting["enabled"]:
            ...
            max_signal = overall_eval.get("attack_indicators", {}).get(
                "max_signal", 0.0
            )
            ...
            metrics.update(
                {
                    ...
                    "privacy_budget_utilization": self._estimate_privacy_budget_usage(
                        config
                    ),
                }
            )
    ...
    def _analyze_scalability_effects(
        self, attack_results: Dict[str, Any], config: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        size_normalization = (
            1.0 + (node_count - 5) * 0.02
        )  # Slight penalty for larger networks
    ...
    def run_comprehensive_experiments(
        ...
        if resume_from is not None:
            if resume_from < 1 or resume_from > len(all_configs):
                raise ValueError(
                    f"Resume point {resume_from} is out of range (1-{len(all_configs)})"
                )
            ...
            try:
                ...
                if results_file.exists():
                    with open(results_file, "r") as f:
                        ...
                        self.logger.info(
                            f"   Loaded {len(previous_results)} previous results_phase1"
                        )
            except Exception as e:
        ...
        if resume_from is not None:
            self.logger.info(
                f"🚀 Resuming {total_experiments} remaining experiments (from {resume_from}/{original_total})"
            )
        else:
            self.logger.info(
                f"🚀 Starting {total_experiments} comprehensive experiments"
            )
        ...
        self.logger.info(
            f"   Estimated total time: {total_estimated_time / 3600:.1f} hours"
        )
        ...
        if max_parallel == 1:
            ...
            for i, config in enumerate(all_configs, 1):
                ...
                self.logger.info(
                    f"[{actual_experiment_num}/{original_total}] Starting experiment..."
                )
                result = self.run_single_experiment(config)
                self.results.append(result)

                # Log brief result summary
                self._log_experiment_result_brief(
                    result, actual_experiment_num, original_total
                )
            ...
            with ProcessPoolExecutor(max_workers=max_parallel) as executor:
                ...
                for future in as_completed(future_to_config):
                    ...
                    try:
                        ...
                        self.logger.error(
                            f"Experiment {actual_experiment_num} failed with exception: {e}"
                        )
                        error_result = self._create_failed_result(
                            config,
                            f"exp_{actual_experiment_num:04d}",
                            str(e),
                            time.time(),
                        )
        ...
        self.logger.info(
            f"✅ All experiments completed in {total_time / 3600:.1f} hours"
        )
    ...
    def _log_experiment_result_brief(
        self, result: Dict[str, Any], current_num: int, total_num: int
    ):
        ...
        dp_desc = (
            f"DP(ε={config['dp_setting'].get('epsilon', 'off')})"
            if config["dp_setting"]["enabled"]
            else "no-DP"
        )
        ...
        if status == "success":
            ...
            self.logger.info(
                f"[{current_num}/{total_num}] {success_icon} {exp_desc} | {dp_desc} | {attack_desc} | "
                f"Attack: {'SUCCESS' if attack_success else 'FAILED'} (conf:{confidence:.3f}) | "
                f"Runtime: {runtime:.1f}s"
            )
        else:
            error_brief = result.get("error", "Unknown error")[
                :100
            ]  # First 100 chars of error
            runtime = result["runtime_seconds"]
            self.logger.info(
                f"[{current_num}/{total_num}] ❌ {exp_desc} | {dp_desc} | {attack_desc} | "
    ...
    def generate_comprehensive_analysis(self):
        ...
        self.logger.info(
            f"Success rate: {len(successful_results) / total_experiments:.1%}"
        )
        ...
        for result in successful_results:
            ...
            row = {
                ...
                "comm_pattern_score": evaluation["attack_type_scores"].get(
                    "Communication Pattern Attack", 0.0
                ),
                "param_magnitude_score": evaluation["attack_type_scores"].get(
                    "Parameter Magnitude Attack", 0.0
                ),
                "topology_structure_score": evaluation["attack_type_scores"].get(
                    "Topology Structure Attack", 0.0
                ),
                # Topology metrics
                "topology_vulnerability": evaluation["topology_specific_metrics"].get(
                    "topology_vulnerability_score", 0.0
                ),
                "communication_complexity": evaluation["topology_specific_metrics"]
                .get("communication_complexity", {})
                .get("total_edges", 0),
                # DP effectiveness
                "dp_protection_effective": evaluation["dp_effectiveness_metrics"].get(
                    "dp_protection_effective", True
                ),
                "signal_strength_with_dp": evaluation["dp_effectiveness_metrics"].get(
                    "signal_strength_with_dp", 0.0
                ),
                # Scalability
                "size_category": evaluation["scalability_metrics"].get(
                    "size_category", "unknown"
                ),
                "attack_scalability_score": evaluation["scalability_metrics"].get(
                    "attack_scalability_score", 0.0
                ),
                # Performance
    ...
    def _generate_key_insights(self, df: pd.DataFrame):
        ...
        if "dp_enabled" in df.columns:
            dp_comparison = df.groupby("dp_enabled")["attack_success"].agg(
                ["mean", "count"]
            )
            ...
            for dp_enabled, stats in dp_comparison.iterrows():
                ...
                self.logger.info(
                    f"   {dp_str}: {stats['mean']:.1%} success rate ({stats['count']} experiments)"
                )
            ...
            if "dp_epsilon" in df.columns:
                dp_strength = (
                    df[df["dp_enabled"]]
                    .groupby("dp_epsilon")["attack_success"]
                    .mean()
                ...
                for epsilon, success_rate in dp_strength.items():
                    self.logger.info(
                        f"     ε={epsilon}: {success_rate:.1%} attack success"
                    )

        # 3. Topology vulnerability ranking
        topo_vuln = (
            df.groupby("topology")["attack_success"]
            .agg(["mean", "count"])
        ...
        for topology, stats in topo_vuln.iterrows():
            self.logger.info(
                f"   {topology}: {stats['mean']:.1%} success rate ({stats['count']} experiments)"
            )
        ...
        for size_cat, success_rate in scale_effects.items():
            self.logger.info(
                f"   {size_cat} networks: {success_rate:.1%} attack success"
            )

        # 5. Attack strategy effectiveness
        strategy_effectiveness = df.groupby("attack_strategy")["attack_success"].agg(
            ["mean", "count"]
        )
        self.logger.info("5. Attack strategy effectiveness:")
        for strategy, stats in strategy_effectiveness.iterrows():
            self.logger.info(
                f"   {strategy}: {stats['mean']:.1%} success rate ({stats['count']} experiments)"
            )

        # 6. Dataset-specific findings
        dataset_findings = df.groupby("dataset")["attack_success"].agg(
            ["mean", "count"]
        )
        self.logger.info("6. Dataset-specific findings:")
        for dataset, stats in dataset_findings.iterrows():
            self.logger.info(
                f"   {dataset}: {stats['mean']:.1%} success rate ({stats['count']} experiments)"
            )

        # 7. FL type comparison
        fl_type_comparison = df.groupby("fl_type")["attack_success"].agg(
            ["mean", "count"]
        )
        self.logger.info("7. FL type comparison:")
        for fl_type, stats in fl_type_comparison.iterrows():
            self.logger.info(
                f"   {fl_type}: {stats['mean']:.1%} success rate ({stats['count']} experiments)"
            )
    ...
    def generate_paper_ready_outputs(self):
        ...
        self.logger.info(
            f"📄 Paper-ready outputs saved in: {self.base_output_dir / 'analysis'}"
        )
            ...
            summary_data.append(
                {
                    ...
                    "Attack Strategy": config["attack_strategy"]
                    .replace("_", " ")
                    .title(),
    ...
    def _create_aggregate_statistics_table(
        self, summary_df: pd.DataFrame
    ) -> pd.DataFrame:
        """Create aggregate statistics table."""

        # Convert Attack Success to numeric
        summary_df["Attack Success Numeric"] = summary_df["Attack Success"].map(
            {"Yes": 1, "No": 0}
        )
...
def main():
    ...
    if args.attack_strategies:
        filters["attack_strategies"] = [
            a.replace("-", "_") for a in args.attack_strategies
        ]
    ...
    phase_name = (
        "Phase 1 (Baseline)" if args.phase == 1 else "Phase 2 (Sampling Effects)"
    )
    ...
    print(
        f"📈 Visualization data in: {args.output_dir}/visualizations_phase{args.phase}"
    )
