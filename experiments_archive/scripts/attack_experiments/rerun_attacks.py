...
def run_attacks_on_visualization_data(viz_dir: str, exp_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"attack_results": [], "status": "failed", "error": str(e)}
...
def rerun_all_attacks(
    ...
    print(
        f"Starting attack re-run on existing Phase {experiment_phase} visualization data..."
    )
    ...
    for i, exp_dir in enumerate(experiment_dirs):
        ...
        if config.get("phase") == 2:
            result_entry["phase2_analysis"] = analyze_phase2_results(
                attack_result, config
            )
    ...
    if failed_experiments:
        print(
            f"  Failed experiments: {failed_experiments[:5]}{'...' if len(failed_experiments) > 5 else ''}"
        )
...
def compare_old_vs_new_metrics(
    ...
    for old_result in old_results:
        ...
        new_result = next(
            (r for r in new_results if r["experiment_name"] == exp_name), None
        )
    ...
    if comparisons:
        ...
        ratios = [
            ...
            if c["improvement_ratio"] != float("inf")
        ]
        ...
        print(
            f"  Average improvement ratio: {sum(ratios) / len(ratios):.2f}x"
            if ratios
            else "N/A"
        )
        ...
        top_improvements = sorted(
            comparisons, key=lambda x: x["improvement"], reverse=True
        )[:5]
        print("\nTop 5 improvements:")
        for comp in top_improvements:
            print(
                f"    {comp['experiment']}: {comp['old_metric']:.6f} → {comp['new_metric']:.6f} ({comp['improvement']:.6f})"
            )


def analyze_phase2_results(
    attack_result: Dict[str, Any], config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    if attack_result["status"] == "success":
        ...
        for attack in attack_result.get("attack_results", []):
            ...
            participation_factor = config.get("client_sampling_rate", 1.0) * config.get(
            ...
            analysis["sampling_impact_metrics"][attack_name] = {
                ...
                "relative_effectiveness": success_metric
                / max(participation_factor, 0.1),  # Avoid division by zero
            }
...
def compare_phase1_vs_phase2(
    ...
    for phase2_result in phase2_results:
        if phase2_result["status"] == "success":
            ...
            if key in phase1_by_config:
                ...
                phase1_metric = get_attack_metric(
                    phase1_result, "Parameter Magnitude Attack"
                )
                phase2_metric = get_attack_metric(
                    phase2_result, "Parameter Magnitude Attack"
                )
                ...
                comparisons.append(
                    {
                        ...
                        "relative_reduction": (phase1_metric - phase2_metric)
                        / max(phase1_metric, 0.001),
                    }
                )
    ...
    if comparisons:
        ...
        for scenario, comps in by_scenario.items():
            ...
            example = comps[0]
            print(
                f"    Example: {example['phase1_metric']:.6f} → {example['phase2_metric']:.6f} (C={example['client_sampling_rate']:.1f}, D={example['data_sampling_rate']:.1f})"
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        if sys.argv[1] == "--compare":
            ...
            print("Usage:")
            print(
                "  python rerun_attacks.py                           # Process all Phase 1 experiments"
            )
            print(
                "  python rerun_attacks.py --phase2                  # Process all Phase 2 experiments"
            )
            print(
                "  python rerun_attacks.py --filter exp_001 exp_002  # Process specific Phase 1 experiments"
            )
            print(
                "  python rerun_attacks.py --filter --phase2 exp_2001 # Process specific Phase 2 experiments"
            )
            print(
                "  python rerun_attacks.py --compare                 # Compare old vs new metrics"
            )
            print(
                "  python rerun_attacks.py --compare-phases          # Compare Phase 1 vs Phase 2 results"
            )
    else:
