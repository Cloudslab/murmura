...
def calculate_correct_reductions():
    ...
    with open(
        "experiments_archive/results/attack_results/realistic_knowledge_full_analysis/realistic_scenario_summary.json",
        "r",
    ) as f:
        results = json.load(f)
        ...
        if scenario_name in results["scenario_effectiveness"]:
            ...
            for attack_name in [
                ...
                if attack_name in scenario_results:
                    ...
                    print(
                        f"{attack_short:24} {success_rate:.1%}          {baseline:.1%}      {reduction:+.1f}%        {status}"
                    )
    ...
    for scenario_name in scenarios:
        if scenario_name in corrected_results:
            effective_attacks = sum(
                1
                for data in corrected_results[scenario_name].values()
                if data["effective"]
            )
            ...
            if effective_attacks == total_attacks:
                ...
                failed_attacks = [
                    attack
                    for attack, data in corrected_results[scenario_name].items()
                    if not data["effective"]
                ]
                print(
                    f"⚠ {scenario_name}: {len(failed_attacks)} attack(s) below threshold: {[a.replace(' Attack', '') for a in failed_attacks]}"
                )
    ...
    with open(
        "experiments_archive/results/attack_results/realistic_knowledge_full_analysis/corrected_reductions.json",
        "w",
    ) as f:
