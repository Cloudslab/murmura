    ...
    if results:
        ...
        for knowledge_level in ["0.0", "0.25", "0.5", "0.75", "1.0"]:
            if knowledge_level in exp["knowledge_results"]:
                ...
                print(
                    f"  Data points visible: {result['visualization_data_summary'].get('parameter_updates', 0)}"
                )
                ...
                for attack in result["attack_results"]:
                    if attack["attack_name"] == "Parameter Magnitude Attack":
                        print(
                            f"  Attack success: {attack.get('attack_success_metric', 0.0):.3f}"
                        )
                        ...
                        if "node_magnitude_stats" in attack:
                            ...
                            if len(stats) >= 2:
                                ...
                                print(
                                    f"  Magnitude range: {min(norms):.3f} - {max(norms):.3f}"
                                )
                                print(
                                    f"  Magnitude spread: {max(norms) - min(norms):.3f}"
                                )
